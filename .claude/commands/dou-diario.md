---
description: Monitoramento diário do DOU previdenciário com criação automática de tarefa no Microsoft To Do
---

# MODO AUTOMÁTICO

Esta rotina foi projetada para rodar em modo headless via cron diário (06:00 BRT em dias úteis) com fallback via SessionStart hook. Aplica os ajustes abaixo ao fluxo padrão.

## Idempotência (executar ANTES do Passo 0)

Primeira camada — state file local. Leia `$CLAUDE_PROJECT_DIR/.claude/state/dou-last-run.txt`. Se o conteúdo é igual à data de hoje em `YYYY-MM-DD` na timezone `America/Sao_Paulo`, abortar imediatamente com a saída exata abaixo e exit code 0:

```
DOU [YYYY-MM-DD] já processado nesta máquina, encerrando.
```

Segunda camada — verificação no Microsoft To Do. Imediatamente antes do POST de criação no Passo 10, consulte a lista alvo com filtro `startswith(title,'DOU [DD/MM/AAAA]')` para a data de hoje. Se já existir tarefa, NÃO criar duplicata. Atualizar o body apenas se o conteúdo divergir do payload calculado nesta execução. Registrar no log "Tarefa pré-existente atualizada" ou "Tarefa pré-existente íntegra, sem mudanças".

## Defaults não-interativos

Nunca pedir confirmação ao usuário. Aplicar defaults seguros sem perguntar:
- Feriado ou recesso, usar último dia útil anterior automaticamente.
- Falha do conector To Do após uma tentativa de refresh, registrar no log e seguir para o Passo 11 (não bloquear).
- Timeout do portal `in.gov.br`, aguardar 30 segundos e tentar de novo, até 3 tentativas, depois desistir e marcar como execução parcial.
- Indicação de skill com ambiguidade entre Rota A (atualizar existente) e Rota B (criar nova), preferir Rota A se houver skill com sobreposição de gatilhos maior que 50%, senão Rota B.

## Persistência de estado

Ao final da execução com sucesso (mesmo parcial — exit code 0 ou 1), gravar a data de hoje em `$CLAUDE_PROJECT_DIR/.claude/state/dou-last-run.txt` no formato `YYYY-MM-DD`. Criar o diretório `.claude/state/` se não existir.

Em caso de falha total (exit code 2), NÃO atualizar o state file. Isso garante que a próxima invocação (cron do dia seguinte ou fallback hook ao abrir sessão) tente de novo.

## Saída em stdout

Ao terminar, imprima exatamente quatro linhas no stdout (após qualquer log interno):

```
DOU [YYYY-MM-DD] - [N] alerta(s), [M] importante(s), [K] informativo(s)
Tarefa To Do: [ID ou PENDENTE-FALHA-GRAPH]
Plugin: [N arquivos modificados ou nenhum]
Log completo: [path]
```

## Exit codes

- `0` — execução completa com sucesso
- `1` — sucesso parcial (algum passo falhou mas tarefa foi criada e/ou houve captura de matéria)
- `2` — falha total (nenhum dado capturado e nenhuma tarefa criada)
...
Use o dropdown "Selecionar Organização Principal" e selecione "Ministério da Previdência Social".
Liste todas as matérias. Para cada uma, abra em nova navegação, leia o conteúdo completo via get_page_text e tire screenshot com save_to_disk: true.
...
PAGINAÇÃO. Se a listagem retornar exatamente 100 itens, a busca pode estar truncada. Repita a mesma URL acrescentando o parâmetro `&start=100`, depois `&start=200` e assim por diante, até obter página com menos de 100 resultados. Some todos os resultados antes de filtrar.
...
Abra o link em nova aba.
Tire screenshot com save_to_disk: true.
Use get_page_text para capturar texto integral.
Identifique normas alteradas, vigência, hipóteses de aplicação e impactos.
Escreva síntese técnica em parágrafos curtos, sem dois pontos, sem travessões.
...
Atualize também a seção "description" do frontmatter da skill, se a alteração ampliar o escopo de gatilhos. Adicione palavras-chave novas (ex: "biometria seguridade social", "Portaria Conjunta MGI/MDS/MPS 23/2026") sem remover gatilhos antigos.
...
11.3. Para qualquer alteração feita no plugin, registre log resumido em arquivo `_base-conhecimento-inss/CHANGELOG-DOU.md` com a data, a skill alterada ou criada, a norma fonte e o link oficial.
...
11.4. Não execute git commit nem push automaticamente. Apenas grave os arquivos. Indique no relatório final que houve modificação e liste os arquivos alterados para revisão humana antes do commit.
