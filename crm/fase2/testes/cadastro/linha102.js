...
(async () => {
  ...
  conf("caso de JEF em primeiro grau também mostra a fila, com o nome da vara",
    await p.evaluate((id) => {
      ...
      k.trf3 = { ordem: 118, total: 1221, grau: 1,
        orgao: "01ª VF Previd. com JEF Cível e Previd. de Catanduva" };
      repintarFicha();
      const e = document.querySelector(".lc-topo .lc-fila");
      const ok = !!e && e.textContent.trim() === "118º"
        && /Catanduva/.test(e.parentElement.getAttribute("title"));
      k.trf3 = antes; repintarFicha(); return ok; }, CASO1));
