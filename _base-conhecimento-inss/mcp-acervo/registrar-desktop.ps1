# Registra o MCP acervo no Claude Desktop (chat e Cowork).
# O app regrava o claude_desktop_config.json com o que tem na memoria, e por isso a entrada
# so persiste se for gravada com o app FECHADO. Rode este script numa janela do PowerShell
...
Write-Host "Feche o Claude pela bandeja (botao direito no icone, Sair). Aguardando..."
while (Get-Process claude -ErrorAction SilentlyContinue | Where-Object Path -like '*WindowsApps*') { Start-Sleep 2 }
Start-Sleep 3

Copy-Item $cfg "$cfg.antes-acervo-$(Get-Date -Format yyyyMMdd-HHmmss).json"
python -c @"
import json
p = r'$cfg'
d = json.load(open(p, encoding='utf-8'))
d.setdefault('mcpServers', {})['acervo'] = {
