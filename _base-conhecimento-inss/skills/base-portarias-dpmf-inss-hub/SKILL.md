---
name: base-portarias-dpmf-inss-hub
description: "Hub completo das Portarias DPMF, DIRBEN e correlatas do INSS, aplicáveis a TODA análise de benefício previdenciário. Use SEMPRE que mencionar Portaria DPMF INSS, Portaria DIRBEN INSS, Portaria 990/2022 CNIS RAC, Portaria 991/2022 concessão revisão, Portaria 992/2022 RMI, Portaria 1316/2025, Portarias 1056/2022 1231/2024 1309/2025 1318/2025 IEA, Portarias 1310/2025 1333/2026 reabilitação, Portaria MPS 462/2026 RICRPS, Portarias 1079/2022 1209/2024 1240/2024 1251/2025 1299/2025 alterações 990, Portaria 19/2026 Teleperícia, Portarias Conjuntas 13 14 15/2026 análise documental B31 B91 B94, Portaria MTP 672/2021 EPI, Portaria 6734/2020 PCMSO, Portaria Conjunta SPS INSS SNAS 2/2014 BPC, Portaria Interministerial 1/2015 IF-BrA, IN 203/2026 vedação novo requerimento processo em curso art 576-A, art 576-A IN 128/2022, prazo recursal 30 dias vedação requerimento, Portaria Conjunta 15/2026 auxílio-acidente análise documental, Resolução 438/2014 revogada. Cruza com cnis-acerto-indicadores, base-erro-administrativo-iea-13975, base-segurado-especial-autodeclaracao-arts-92-93-94, peticao-previdenciaria, precedentes-previdenciarios."
---

# Hub das Portarias DPMF, DIRBEN e correlatas. INSS

## 1. Quando acionar esta skill

Acione SEMPRE que houver dúvida sobre o regramento administrativo aplicável a qualquer benefício, ato administrativo, ratificação documental, processo de concessão, revisão, recurso ou perícia.

A skill é fonte primária para identificar QUAL portaria rege QUAL aspecto do procedimento, com numeração, finalidade central, dispositivos-chave e intersecções com Lei e jurisprudência.

A skill aplica-se transversalmente a todas as análises de benefício previdenciário.

## 2. Marco normativo

CF/88, art. 5º, II (legalidade).

CF/88, art. 84, IV (regulamentação por decreto).

Lei 8.213/91 e Lei 8.212/91 (Lei Básica).

Decreto 3.048/99 (Regulamento da Previdência Social).

IN PRES/INSS 128/2022 (Instrução Normativa-mãe).

Conjunto de portarias DPMF, DIRBEN, MPS e MTP detalhadas a seguir.

## 3. Hierarquia normativa do INSS

Constituição Federal.

Leis complementares (LC 142/2013, LC 155/2016).

Leis ordinárias (Lei 8.213/91, Lei 8.212/91, Lei 13.846/2019).

Decretos regulamentares (Decreto 3.048/99).

Instruções Normativas (IN PRES/INSS 128/2022).

Portarias (DPMF, DIRBEN, MPS).

Enunciados do CRPS.

Orientações internas do INSS.

A Portaria DPMF/DIRBEN não pode reduzir o alcance da Lei nem de precedente vinculante (Súmula, Tema repetitivo, Tema TNU).

Toda exigência da Portaria que extrapole a Lei é argumento de derrotabilidade em recurso ou ação.

## 4. Portaria DPMF/INSS 990/2022. Acerto de CNIS, indicadores e RAC

Disciplina os procedimentos de acerto do Cadastro Nacional de Informações Sociais (CNIS), os indicadores que sinalizam pendências e os formulários de Requerimento de Atualização Cadastral (RAC).

Arts. 90 a 94. Segurado especial e autodeclaração rural. Detalhamento completo em `base-segurado-especial-autodeclaracao-arts-92-93-94`.

Anexos. Formulários I, I-A, I-B, I-C, I-D, I-E, I-F (RACs específicos).

Indicadores bloqueantes. PEXT, PREC-MENOR-MIN, PDIV-DADOS-GFIP, PVIN-IRREG, PREC-FBR, PADM-EMPR.

Detalhamento em `cnis-acerto-indicadores`.

Alterações relevantes.

Portaria DIRBEN 1005/2022. Ajustes iniciais.

Portaria DIRBEN 1079/2022. Inclusão do art. 94, §5º (cláusula de flexibilização da autodeclaração rural).

Portaria DIRBEN 1209/2024. Inclusão do inciso XXXVI no art. 93 (declaração do INCRA para quilombolas).

Portaria DIRBEN 1240/2024. Alterações no art. 92 (bases governamentais) e formulários RAC.

Portaria DIRBEN 1251/2025. Alterações nos arts. 3º, 20, 21 e 22.

Portaria DIRBEN 1299/2025. Inclusão de regras para indígenas (incisos X e XII).

Portaria 1316/2025. Alterações pontuais.

Aplicação transversal. Todo análise de benefício envolve verificação prévia do CNIS, identificação de indicadores e eventual acerto via RAC.

## 5. Portaria DPMF/INSS 991/2022. Concessão e revisão administrativa de benefícios

Disciplina os procedimentos de concessão e revisão administrativa de benefícios.

Aplicação. Todo requerimento administrativo de benefício é regido pela 991. Inclui prazos, fluxos, automatização (concessão automática, indeferimento automático), exigências documentais, decisões.

Arts. 1º a 5º. Princípios. Boa-fé objetiva, devido processo administrativo, motivação, eficiência.

Arts. 6º a 30. Procedimento de concessão. Requerimento, instrução, perícia, decisão.

Arts. 31 a 60. Revisão administrativa. Iniciativa do segurado, iniciativa de ofício, prazo decadencial, efeitos.

Arts. 61 a 100. Diretrizes específicas por espécie de benefício.

Aplicação transversal. Toda análise de benefício é orientada pela Portaria 991.

## 6. Portaria DPMF/INSS 992/2022. Cálculo da Renda Mensal Inicial (RMI)

Disciplina os critérios de cálculo da RMI dos benefícios previdenciários.

Aplicação. Indispensável para análise de questões de cálculo, atividade concomitante (Tema 1.070/STJ), revisão da vida toda (Tema 1.102/STF), revisão do teto, revisão do art. 29 II, IRSM e ORTN/OTN.

Arts. 1º a 20. Disposições gerais. Salários-de-contribuição, divisor mínimo, atualização monetária.

Arts. 21 a 50. Cálculo por espécie. Aposentadorias, pensão, auxílio-doença, BPC.

Arts. 51 a 100. Atividades concomitantes. Tema 1.070/STJ.

Anexos. Tabelas e exemplos.

Aplicação transversal. Análise crítica de RMI em concessão e revisão.

## 7. Portarias DIRBEN sobre Indicação de Erro Administrativo (IEA)

Conjunto de portarias que regulamenta o instrumento interno da IEA (código 13975).

Portaria DIRBEN/INSS 1.056/2022. Disciplina originária da Supervisão Técnica em Benefícios e da IEA.

Portaria DIRBEN/INSS 1.231, de 15/10/2024 (DOU 06/11/2024). Conferiu ao art. 13 a redação atualmente invocada para o rol exaustivo de chefias legitimadas e para os prazos internos.

Portaria DIRBEN/INSS 1.309, de 21/10/2025 (DOU 04/11/2025). Revogou expressamente as Portarias 1.056/2022 e 1.231/2024 (art. 44, III e V), passando a regular o tema. Reproduziu integralmente o conteúdo do art. 13 §1º no art. 26 §1º, com inclusão da nova espécie de Supervisão Técnica em Benefícios por Incapacidade (código 19056).

Portaria DIRBEN/INSS 1.318, de 19/12/2025. Alterou pontualmente a Portaria 1.309/2025.

Detalhamento completo em `base-erro-administrativo-iea-13975`.

## 8. Portarias DIRBEN sobre Reabilitação Profissional

Portaria DIRBEN/INSS 1.310/2025. Disciplina a Reabilitação Profissional. Programa de Reabilitação (PR), profissões compatíveis, sociais.

Portaria DIRBEN/INSS 1.333/2026. Alterações e atualizações da reabilitação.

Aplicação. Sempre que houver auxílio-doença prolongado, decisão sobre invalidez parcial, encaminhamento à reabilitação. Combinar com Súmula 47/TNU (condições pessoais e sociais).

## 9. Portaria MPS 462/2026. Novo Regimento Interno do CRPS (RICRPS)

Disciplina o procedimento recursal administrativo no Conselho de Recursos da Previdência Social.

Substitui o RICRPS anterior. Alterações em prazos, fungibilidade recursal, admissibilidade, instrução.

Aplicação. Todo recurso administrativo (recurso ordinário à JR, recurso especial à CAJ, PUJ, Reclamação ao Pleno) é regido pela 462/2026.

Detalhamento em skills correlatas (admissibilidade, recursos superiores, incidentes).

## 10. Portarias Conjuntas MPS/INSS 13, 14 e 15 de 23/03/2026. Análise Documental

Portaria Conjunta MPS/INSS 13/2026. Análise documental para B31 (auxílio-doença).

Portaria Conjunta MPS/INSS 14/2026. Análise documental para B91 (aposentadoria por invalidez).

Portaria Conjunta MPS/INSS 15/2026. Análise documental para B94 (auxílio-acidente).

Aplicação. Regime de concessão sem perícia presencial baseado em análise documental rigorosa. Limite 30 dias de afastamento sem perícia. Trava 180 dias para análise documental sucessiva. Trava três indeferimentos consecutivos.

Detalhamento em `analise-documental-incapacidade`.

## 11. Portaria DPMF/INSS 19/2026. Teleperícia

Disciplina a Teleperícia. Modalidade de perícia médica e biopsicossocial por videoconferência.

Aplicação. Casos de menor complexidade. Casos complexos exigem presencial. Recusa pode ser fundamentada por cerceamento de defesa.

## 12. Portaria MTP 672/2021. Avaliação de EPI

Disciplina os procedimentos de obtenção, renovação e cancelamento do Certificado de Aprovação (CA) de Equipamentos de Proteção Individual (EPI).

Aplicação. Análise técnica da eficácia/efetividade de EPI em aposentadoria especial e em casos de agente nocivo.

Detalhamento em `base-tema383-tnu-fundacentro-epi-biologicos` e `base-especial-epi`.

## 13. Portaria MTP 6.734/2020. PCMSO

Disciplina o Programa de Controle Médico de Saúde Ocupacional (PCMSO).

Aplicação. Documentação ambiental para B31, B91, B92, B94 e aposentadoria especial. Cruza com NR-32 e PGR.

## 14. Portaria Interministerial AGU/MPS/MF/MP/PR nº 1, de 27/01/2015

Disciplina a avaliação biopsicossocial para aposentadoria PCD da LC 142/2013.

Sete domínios. Função e estrutura do corpo, atividades e participação, fatores ambientais, fatores pessoais, IF-BrA e lógica fuzzy.

Aplicação. Análise de aposentadoria PCD. Detalhamento em `base-pcd-if-bra-metodologia` e `base-aposentadoria-pcd-lc142`.

## 15. Portaria Conjunta SPS/INSS/SNAS nº 2, de 19/09/2014

Disciplina a avaliação biopsicossocial do BPC/LOAS.

TCQ (Teste de Conjugação Qualitativa). Lógica de avaliação multidisciplinar.

Aplicação. Análise do BPC para PCD. Detalhamento em `base-bpc-impedimento-longo-prazo` e `analise-bpc-loas`.

## 16. Tabela síntese das portarias por tema

| Tema | Portaria | Skill detalhada |
|---|---|---|
| CNIS, indicadores, RAC | DPMF/INSS 990/2022 (e alterações) | cnis-acerto-indicadores |
| Concessão e revisão administrativa | DPMF/INSS 991/2022 | aplicação transversal |
| Cálculo da RMI | DPMF/INSS 992/2022 | base-calculo-rmi-ec103 |
| Indicação de Erro Administrativo (IEA) | DIRBEN 1056, 1231, 1309, 1318 | base-erro-administrativo-iea-13975 |
| Reabilitação Profissional | DIRBEN 1310/2025, 1333/2026 | base-reabilitacao-profissional-portaria-1310-1333 |
| RICRPS (CRPS) | MPS 462/2026 | admissibilidade-barreiras-crps |
| Análise Documental B31 | Conjunta MPS/INSS 13/2026 | analise-documental-incapacidade |
| Análise Documental B91 | Conjunta MPS/INSS 14/2026 | analise-documental-incapacidade |
| Análise Documental B94 | Conjunta MPS/INSS 15/2026 | analise-documental-incapacidade |
| Teleperícia | DPMF/INSS 19/2026 | analise-documental-incapacidade |
| Autodeclaração rural | DPMF/INSS 990 arts. 92-94 (e alterações) | base-segurado-especial-autodeclaracao-arts-92-93-94 |
| EPI (CA) | MTP 672/2021 | base-especial-epi, base-tema383-tnu-fundacentro-epi-biologicos |
| PCMSO | MTP 6734/2020 | (transversal em saúde ocupacional) |
| IF-BrA aposentadoria PCD | Interministerial 1/2015 | base-pcd-if-bra-metodologia |
| BPC biopsicossocial | Conjunta SPS/INSS/SNAS 2/2014 | base-bpc-impedimento-longo-prazo |

## 17. Aplicação a cada análise de benefício

### 17.1. Aposentadoria por idade

Verificar CNIS e indicadores via Portaria 990/2022. Aplicar Portaria 991/2022 para procedimento. Aplicar Portaria 992/2022 para cálculo da RMI. Em caso de período rural, aplicar arts. 92-94 da 990 (autodeclaração).

### 17.2. Aposentadoria por tempo de contribuição

Mesmo fluxo. Adicionalmente verificar reafirmação DER e melhor benefício.

### 17.3. Aposentadoria especial

Portarias correlatas. Portaria 990/2022 para CNIS. Portaria 991/2022 para procedimento. Portaria MTP 672/2021 para EPI.

### 17.4. Aposentadoria PCD

Portarias correlatas. Portaria Interministerial 1/2015 para IF-BrA. Portaria 991/2022 para procedimento.

### 17.5. Auxílio-doença, aposentadoria por invalidez, auxílio-acidente

Portarias correlatas. Portarias Conjuntas MPS/INSS 13, 14 e 15/2026 para análise documental. Portaria DPMF/INSS 19/2026 para Teleperícia. Portaria 991/2022 para procedimento. Portarias DIRBEN 1310/2025 e 1333/2026 para reabilitação.

### 17.6. Pensão por morte

Portaria 991/2022 para procedimento. Portaria 992/2022 para cálculo.

### 17.7. BPC/LOAS

Portaria Conjunta SPS/INSS/SNAS 2/2014 para avaliação biopsicossocial. Portaria 991/2022 para procedimento.

### 17.8. Salário-maternidade

Portaria 991/2022 para procedimento. Em rural, arts. 92-94 da 990 (autodeclaração).

### 17.9. Auxílio-reclusão

Portaria 991/2022 para procedimento.

### 17.10. Recurso administrativo CRPS

Portaria MPS 462/2026 para o RICRPS.

### 17.11. Erro administrativo (IEA)

Portarias DIRBEN 1056/2022, 1231/2024, 1309/2025, 1318/2025.

## 18. Estratégias pró-segurado em cada portaria

### 18.1. Em geral

A Portaria não vincula o juízo. Em sede judicial, é apenas referência.

Toda exigência da Portaria que extrapole a Lei é derrotável.

Em recurso administrativo, atacar tanto o ato concreto quanto a interpretação restritiva da própria Portaria.

### 18.2. Portaria 990/2022

Em CNIS com indicadores, aplicar acerto via RAC. Ver `cnis-acerto-indicadores`.

Em autodeclaração rural, aplicar §5º do art. 94 sempre que possível. Ver `base-segurado-especial-autodeclaracao-arts-92-93-94`.

### 18.3. Portaria 991/2022

Em mora administrativa, aplicar prazos do art. 49 da Lei 9.784/1999, art. 41-A §5º da Lei 8.213/1991 e parâmetro do RE 1.171.152/SC (90 dias).

Em silêncio administrativo, MS por omissão.

### 18.4. Portaria 992/2022

Em RMI inferior à esperada, verificar atividade concomitante (Tema 1.070/STJ), revisão da vida toda (Tema 1.102/STF), revisão do teto, art. 29 II, IRSM, ORTN/OTN.

### 18.5. Portarias DIRBEN 1056-1309 (IEA)

Provocar nominalmente as autoridades legitimadas a abrir IEA. Ver `base-erro-administrativo-iea-13975`.

### 18.6. Portarias Conjuntas 13, 14, 15/2026

Em análise documental rejeitada, requerer Teleperícia. Em recusa de Teleperícia, fundamentar como cerceamento de defesa.

### 18.7. Portaria 19/2026 (Teleperícia)

Em casos complexos, requerer perícia presencial.

## 19. Documentos institucionais consultáveis

Diário Oficial da União (DOU) para todas as Portarias.

Portal do INSS (gov.br/inss). Para Portarias DPMF e DIRBEN.

Portal do MPS (gov.br/previdencia). Para Portaria MPS 462/2026.

Portal do MTE (gov.br/trabalho-e-emprego). Para Portarias MTP 672/2021 e MTP 6.734/2020.

Portal Planalto. Para Decretos e Leis.

Portal SciELO. Para doutrina aplicada.

Portal CRPS. Para Resoluções e Enunciados.

## 20. Integração com outras skills

Detalhamento de cada portaria em skills específicas. Esta skill é hub orquestrador.

CNIS. `cnis-acerto-indicadores`.

Autodeclaração rural. `base-segurado-especial-autodeclaracao-arts-92-93-94`.

IEA. `base-erro-administrativo-iea-13975`.

Análise documental B31/B91/B94. `analise-documental-incapacidade`.

Reabilitação profissional. `base-reabilitacao-profissional-portaria-1310-1333`.

RMI. `base-calculo-rmi-ec103`.

CRPS. `admissibilidade-barreiras-crps`, `incidentes-instrucao-crps`, `recursos-superiores-crps`.

EPI. `base-especial-epi`, `base-tema383-tnu-fundacentro-epi-biologicos`.

BPC. `analise-bpc-loas`, `base-bpc-impedimento-longo-prazo`.

Aposentadoria PCD. `aposentadoria-deficiencia`, `base-pcd-if-bra-metodologia`.

Triagem. `triagem-caso-novo`.

Petição. `peticao-previdenciaria`.

Auditoria. `revisao-peticao`.

Precedentes. `precedentes-previdenciarios`.

Fungibilidade. `base-fungibilidade-previdenciaria`.

## 21. Alertas

Primeiro, sempre verificar a vigência das Portarias antes de citar (DOU).

Segundo, em recurso administrativo, citar a Portaria como suporte. Em ação judicial, a Portaria não vincula o juízo.

Terceiro, toda Portaria que reduzir a Lei é derrotável.

Quarto, monitorar alterações sucessivas. Portarias DIRBEN são frequentemente alteradas.

Quinto, em caso de Portaria nova, verificar revogação de Portaria anterior (compatibilidade temporal).

Sexto, em todo procedimento administrativo, identificar QUAL Portaria rege QUAL aspecto.

Sétimo, em estratégia processual, mapear todas as Portarias aplicáveis ao caso e usar como reforço normativo.

## 22. Doutrina de apoio

Frederico Amado. Direito Previdenciário.

Hugo Goes. Manual de Direito Previdenciário.

Wladimir Novaes Martinez. Curso de Direito Previdenciário.

Carlos Alberto Pereira de Castro e João Batista Lazzari. Manual de Direito Previdenciário.

Marina Vasques Duarte. Direito Previdenciário.

Marco Aurélio Serau Junior. Direito Previdenciário e Processo Judicial Previdenciário.

Daniel Pulino. Aposentadoria por Invalidez.

José Antônio Savaris. Direito Processual Previdenciário.

## Atualização DOU 30/04/2026

### IN PRES/INSS nº 203, de 22 de abril de 2026
Norma: Instrução Normativa PRES/INSS, nº 203, 22/04/2026
Órgão: INSS — Presidência
Vigência: 24 de abril de 2026
Publicação no DOU: 24/04/2026, Seção 1
Link oficial: https://legisjet.com.br/conteudo/instrucao-normativa-presinss-n-203-de-22-de-abril-de-2026-dou-de-24-04-2026/

Resumo da alteração. A IN 203/2026 acrescenta o art. 576-A à IN PRES/INSS 128/2022 (Instrução Normativa-mãe), vedando expressamente a apresentação de novo requerimento pelo interessado enquanto houver processo em curso referente à mesma espécie de benefício. O parágrafo primeiro define que o processo permanece em curso enquanto não transcorrido o prazo de 30 dias para interposição de recurso administrativo. Único exceção: o pedido de revisão do benefício é admissível mesmo com processo pendente (parágrafo segundo). A norma revoga a Resolução nº 438/PRES/INSS, de 03/09/2014, que regulava a organização do atendimento nas agências INSS quanto a documentos de identificação, biometria e regras de agendamento.

Impacto na advocacia. Toda a estratégia de recuperação de documentação complementar pós-indeferimento precisa ser revisada. Até 24/04/2026, era possível reapresentar novo requerimento imediatamente após indeferimento, juntando os documentos faltantes. A partir de 24/04/2026, isso é vedado enquanto correr o prazo recursal de 30 dias. O escritório deve implantar controle de prazo de 30 dias a partir da ciência do indeferimento para todos os casos ativos. Em caso de indeferimento com documentação incompleta, a rota mais eficiente é o recurso ao CRPS dentro dos 30 dias, juntando novos documentos em fase recursal, preservando assim a DIB mais favorável. Acionar `ponte-workflow-crps`. A rota alternativa de novo requerimento só se abre após o transcurso dos 30 dias, resultando em DIB posterior.

### Portaria Conjunta MPS/INSS nº 15, de 23 de março de 2026
Norma: Portaria Conjunta MPS/INSS, nº 15, 23/03/2026
Órgão: Ministério da Previdência Social e INSS
Vigência: 24 de março de 2026
Publicação no DOU: 24/03/2026, Seção 1

Resumo da alteração. Institui análise documental prévia para requerimentos de auxílio-acidente (art. 86 da Lei 8.213/91). A Perícia Médica Federal verifica documentalmente se há acidente, sequela e redução permanente da capacidade laborativa antes de agendar perícia presencial. Documentação mínima exigida: documento oficial com foto, documentação médica legível com descrição da lesão, data do acidente, nexo causal, identificação do profissional com registro no Conselho de Classe e assinatura (eletrônica aceita). O indeferimento por análise documental não encerra o processo, admitindo recurso ao CRPS. Cruza com `base-b94-nexo-acidentario-ntep` e `base-auxilio-acidente-b94-pos-reforma`.

## 23. O que NÃO está nesta skill

Detalhamento operacional de cada Portaria. Está em skills específicas.

Análise de procedimentos não previdenciários.

Cálculo concreto de RMI. Está em skills de cálculo.

Texto integral das Portarias. Consultar fontes oficiais.

## 24. Fontes

Consulte os arquivos `references/FUNDAMENTOS-E-CENARIOS.md` e `references/JURISPRUDENCIA-E-REFUTACAO.md`.
