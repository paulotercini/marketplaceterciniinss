---
name: base-revisao-irsm-fevereiro-1994
description: "Revisão pelo IRSM de fevereiro/1994 (39,67%) sobre os salários de contribuição anteriores à conversão em URV, com fundamento no art. 21 da Lei 8.880/1994 e na jurisprudência consolidada da Terceira Seção do STJ (REsp 523.680/SP). Use SEMPRE que mencionar revisão do IRSM, revisão pelo IRSM de fevereiro de 1994, IRSM 39,67%, Índice de Reajuste do Salário Mínimo, Plano Real, conversão para URV, Lei 8.880/1994, art. 21 Lei 8.880, IRSM 39,67 por cento, jurisprudência consolidada da Terceira Seção do STJ, salários de contribuição anteriores a março de 1994, recomposição de salários de contribuição, correção inflacionária SC, RMI com IRSM, benefícios com DIB entre março de 1994 e fevereiro de 1998, decadência IRSM, prescrição quinquenal parcelas IRSM, revisão reflexa em pensão por morte. Cruza com decadencia-revisao-previdenciaria, peticao-previdenciaria, revisao-peticao, base-calculo-rmi-ec103, base-revisao-teto-buraco-negro-verde e precedentes-previdenciarios."
---

# Revisão pelo IRSM de Fevereiro/1994

## Escopo

Skill temática pró-segurado. Orienta a discussão da revisão pelo IRSM de fevereiro de 1994, que determina a aplicação do índice de 39,67% sobre os salários de contribuição anteriores à conversão em URV, para recomposição da base de cálculo dos benefícios concedidos no período de transição para o Plano Real.

## Marco normativo central

CF/88, art. 201, §2º. Preservação do valor real do benefício.

Lei 8.880/1994, art. 21. Conversão dos salários em URV e tratamento dos reajustes pretéritos.

Lei 8.213/91, art. 29. Cálculo do Salário de Benefício.

Lei 8.213/91, art. 103. Decadência decenal.

## Marco jurisprudencial

NÃO EXISTE tema repetitivo do IRSM (conferido pelo Paulo na fonte oficial em 04/07/2026), e o REsp 1.168.657 antes citado é tributário (parcelamento), sem relação com a matéria. O fundamento é a jurisprudência CONSOLIDADA da Terceira Seção do STJ, ementa representativa no REsp 523.680/SP (Quinta Turma, Rel. Min. Jorge Scartezzini, DJ 24/05/2004), 'Na atualização monetária dos salários-de-contribuição, para fins de apuração da renda mensal inicial do benefício, deve ser aplicado o IRSM integral do mês de fevereiro/94, da ordem de 39,67%'. Leading cases, REsp 163.754/SP, REsp 245.148/SC e REsp 226.909/SC. Base normativa, art. 21, §1º, da Lei 8.880/94 e art. 41, II, da Lei 8.213/91.

Fonte oficial em https://www.stj.jus.br

Aplicação pacificada na jurisprudência (não há repetitivo; a força vem da consolidação na Terceira Seção).

Súmula 85 STJ. Prescrição quinquenal das parcelas.

## Espaço pró-segurado

Primeiro, benefícios concedidos a partir de março de 1994, cujos salários de contribuição anteriores não tiveram aplicação do IRSM de 39,67%.

Segundo, benefícios concedidos entre março de 1994 e fevereiro de 1998, especialmente os que possuem salários de contribuição entre 1989 e fevereiro de 1994, período em que a distorção foi mais relevante.

Terceiro, pensões derivadas de benefícios revisáveis, com direito à revisão reflexa.

Quarto, revisão administrativa em curso, em que o INSS aplicou o IRSM em percentual inferior ao devido ou deixou de aplicar sobre todos os salários cabíveis.

## Decadência e prescrição

Art. 103 da Lei 8.213/91. Decadência decenal do primeiro pagamento.

Súmula 85 STJ. Prescrição quinquenal das parcelas.

Atenção. Benefícios concedidos há mais de 10 anos, em regra, estão com decadência consumada. Para benefícios mais recentes, prazo em curso.

## Regra pró-segurado

Primeiro, obter a Carta de Concessão e a memória administrativa com a evolução dos salários de contribuição.

Segundo, aferir se o INSS aplicou o IRSM de 39,67% sobre todos os salários de contribuição anteriores à conversão em URV.

Terceiro, calcular a diferença na RMI.

Quarto, verificar prazo decadencial e prescrição quinquenal.

Quinto, requerimento administrativo antes do ajuizamento (Tema 1124 STJ).

Sexto, ação judicial com pedido de readequação e pagamento de diferenças.

## Integração com outras skills

Ao redigir peça, acionar `peticao-previdenciaria`.
Ao simular cálculo, acionar `base-calculo-rmi-ec103`.
Ao verificar decadência, acionar `decadencia-revisao-previdenciaria`.
Ao buscar precedentes, acionar `precedentes-previdenciarios`.
Em cumulação com revisão do teto, acionar `base-revisao-teto-buraco-negro-verde`.
Em revisão geral, acionar `revisao-peticao`.

## Alertas

Primeiro, após o Tema 415 STJ, o INSS passou a aplicar o IRSM administrativamente. Benefícios concedidos após 2010 tendem a já ter a aplicação. Verificar concretamente.

Segundo, decadência decenal é crítica.

Terceiro, a revisão só vale se houver distorção concreta.

Quarto, vedação de cumulação indevida com reajustes já aplicados (vedação de bis in idem).

## Hub de portarias administrativas

Hub das Portarias DPMF/DIRBEN/INSS aplicáveis a este benefício. Acionar `base-portarias-dpmf-inss-hub` para identificar quais Portarias regem o procedimento administrativo, o cálculo, as ratificações e os recursos no caso concreto.

## Doutrina de apoio

Wladimir Novaes Martinez, análise da conversão em URV.

Frederico Amado, Tema 415 STJ.

Hugo Goes, cálculo com IRSM.

Fábio Zambitte Ibrahim, preservação do valor real.

Daniel Pulino, segurança jurídica em regras de transição monetária.

Marco Aurélio Serau Junior, análise do Plano Real.

IBDP, defesa do IRSM.

## O que NÃO está nesta skill

RVT está em `base-revisao-vida-toda-rvt`. Revisão do teto está em `base-revisao-teto-buraco-negro-verde`. Melhor benefício do art. 29 II está em `base-revisao-art29-melhor-beneficio`. Reajustes ORTN/OTN estão em `base-revisao-reajuste-ortn-otn`.
