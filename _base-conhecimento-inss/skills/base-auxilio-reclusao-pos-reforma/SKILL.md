---
name: base-auxilio-reclusao-pos-reforma
description: "Auxílio-reclusão B25 pós EC 103/2019, ótica pró-segurado. Use SEMPRE que mencionar auxílio-reclusão, B25, dependentes de preso, segurado recluso, baixa renda preso, art. 80 Lei 8.213, MP 871/2019, Tema 1162 STJ, Tema 896 STJ, Tema 357 TNU, Tema 310 TNU, Tema 89 STF, flexibilização critério renda, média 12 salários prisão, divisor fixo 12, progressão regime auxílio-reclusão, monitoramento eletrônico, cessação auxílio-reclusão, carência 24 contribuições reclusão, certidão judicial prisão, EC 103/2019 auxílio-reclusão, teto baixa renda, desempregado preso renda zero, tempus regit actum reclusão, direito adquirido progressão regime, regime semiaberto preso, REsp 2240220, REsp 2256869, IRDR 35 TRF4, Lei 13.846/2019. Cruza com auxilio-reclusao-previdenciario, peticao-previdenciaria, precedentes-previdenciarios, ms-competencia-autoridade-coatora, periodo-graca-qualidade-segurado e triagem-caso-novo."
---

# Auxílio-Reclusão (B25) Pós EC 103/2019

## Escopo

Skill temática pró-segurado do acervo do escritório Paulo Roberto Tercini Filho. Orienta a concessão, o rateio, a cessação e a revisão do auxílio-reclusão sob o regime pós MP 871/2019 e pós EC 103/2019. Objetivo único é defender os dependentes do segurado recluso.

## Marco normativo central

Lei 8.213/91, art. 80. Concessão do auxílio-reclusão aos dependentes do segurado de baixa renda recolhido à prisão em regime fechado.

Lei 8.213/91, art. 80, §1º. Concessão independe de carência quanto ao óbito, mas a MP 871/2019 e Lei 13.846/2019 passaram a exigir 24 contribuições mensais.

Lei 8.213/91, art. 25, IV. Carência de 24 contribuições para auxílio-reclusão.

MP 871/2019, convertida na Lei 13.846/2019. Alterações substanciais. Introdução da carência de 24 contribuições. Alteração do critério de baixa renda. Restrição ao regime fechado.

EC 103/2019. Não alterou estruturalmente o B25, mantendo o regime da Lei 13.846/2019.

Decreto 3.048/99, art. 116. Detalha o B25.

Portaria MPS que define o teto de baixa renda (valor atualizado periodicamente).

## Requisitos cumulativos

Primeiro, qualidade de segurado na data da prisão.

Segundo, baixa renda do segurado na data da prisão (último salário-de-contribuição).

Terceiro, carência de 24 contribuições (após Lei 13.846/2019).

Quarto, recolhimento à prisão em regime fechado (após Lei 13.846/2019, que excluiu regime semiaberto).

Quinto, dependentes no sentido do art. 16.

## Cenários operacionais pró-segurado

Primeiro, segurado preso antes da Lei 13.846/2019. Tempus regit actum. Aplica regra anterior, com regime semiaberto admitido e sem exigência de 24 contribuições.

Segundo, segurado desempregado preso. REsp 2.240.220 STJ reconheceu que renda zero atende o critério de baixa renda. Tese pró-dependente.

Terceiro, segurado com renda ligeiramente acima do teto mas com dependentes em situação de vulnerabilidade. Tema 896 STJ flexibiliza critério em casos excepcionais.

Quarto, segurado em regime fechado com progressão posterior. Direito adquirido à manutenção do benefício (segundo corrente pró-dependente).

Quinto, segurado em monitoramento eletrônico (tornozeleira). Discussão sobre equiparação ao regime fechado. Tese pró-segurado em reconstrução.

Sexto, DIB retroativa à data da prisão se requerida em até 180 dias para dependente menor de 16 anos, ou 90 dias para demais. Senão, DIB na DER.

Sétimo, média de 12 salários imediatamente anteriores à prisão (divisor fixo 12) para verificação do critério de baixa renda (Tema 357 TNU).

## Regra e estratégia

A regra geral é que o B25 é devido aos dependentes enquanto o segurado estiver preso em regime fechado. O INSS tende a negar por critério de renda ou por ausência de carência. Enfrentar.

Primeiro, verificar data da prisão para definir regime aplicável (antes ou depois da Lei 13.846/2019).

Segundo, em segurado desempregado, invocar REsp 2.240.220 STJ (renda zero = baixa renda).

Terceiro, em renda ligeiramente acima, invocar Tema 896 STJ (flexibilização excepcional).

Quarto, em progressão de regime, invocar direito adquirido e manutenção do benefício.

Quinto, manter documentação atualizada de permanência no regime fechado (atestado carcerário).

## Integração com outras skills

Ao redigir peça, acione `peticao-previdenciaria`.
Ao verificar qualidade, acione `periodo-graca-qualidade-segurado`.
Ao buscar precedentes, acione `precedentes-previdenciarios`.
Acionar skill do escritório `auxilio-reclusao-previdenciario` para técnica processual.
Acionar `ms-competencia-autoridade-coatora` em MS.

## Alertas

Primeiro, Lei 13.846/2019 restringiu muito o B25. Atuar tempestivamente para enquadrar caso a regime anterior quando a prisão for pretérita.

Segundo, critério de baixa renda é pelo salário-de-contribuição do segurado na prisão, não pela renda do dependente (distinção crítica).

Terceiro, progressão de regime é controvérsia viva. Tese pró-dependente sustenta manutenção por direito adquirido.

Quarto, monitoramento eletrônico isolado (sem prisão) não gera B25.

## Fungibilidade aplicável

Fungibilidade previdenciária. Acionar `base-fungibilidade-previdenciaria` para análise da relação entre este benefício e outros eventualmente cabíveis (REsp 2.246.096/MG, Tema 217 TNU, Tema 1018 STJ, Tema 995 STJ, vedação à conversão prejudicial).

## Hub de portarias administrativas

Hub das Portarias DPMF/DIRBEN/INSS aplicáveis a este benefício. Acionar `base-portarias-dpmf-inss-hub` para identificar quais Portarias regem o procedimento administrativo, o cálculo, as ratificações e os recursos no caso concreto.

## Autodeclaração rural

Autodeclaração rural e ratificação pelos arts. 92, 93 e 94 da Portaria 990/2022. Acionar `base-segurado-especial-autodeclaracao-arts-92-93-94` quando houver período de atividade rural em regime de economia familiar a comprovar (segurado especial).

## Doutrina de apoio

Frederico Amado, em Direito Previdenciário, sustenta interpretação extensiva do conceito de baixa renda e proteção dos dependentes.

Hugo Goes, em Manual, detalha os requisitos pós Lei 13.846/2019.

Fábio Zambitte Ibrahim, em Curso, critica a restrição ao regime fechado e defende manutenção em progressão.

Wladimir Novaes Martinez analisa o B25 e defende a flexibilização do critério de renda.

O IBDP sustenta institucionalmente o direito dos dependentes à proteção durante a reclusão.

## O que NÃO está nesta skill

Não está aqui a pensão por morte, objeto de `base-pensao-por-morte-pos-reforma`. Não estão aqui os detalhes processuais de pensão, objeto de `pensao-por-morte`.
