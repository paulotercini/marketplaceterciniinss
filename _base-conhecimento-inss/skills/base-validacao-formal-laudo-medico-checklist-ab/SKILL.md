---
name: base-validacao-formal-laudo-medico-checklist-ab
description: "Validação formal de relatório médico pelos Checklists A e B do escritório, regime CFM e Portarias MPS/INSS 13/2026, 14/2026, 15/2026. Use SEMPRE que mencionar checklist A laudo médico, checklist B laudo PCD BPC, validação formal laudo, auditoria documental relatório médico, Resolução CFM 1.851/2008, Resolução CFM 2.314/2022 telemedicina, IN 128/2022 art. 286 287 290, B31 laudo, B91 laudo, B92 laudo, B94 laudo, BPC laudo, PCD laudo IF-BrA, IFBrM laudo, laudo extemporâneo, validade temporal laudo, prognóstico laudo, exames complementares, CID laudo, dupla codificação CID DSM-5-TR autismo, fibromialgia laudo Lei 15.176, perícia documental Portaria 13/2026 14/2026 15/2026, parecer verossimilhança, Lei 12.842/2013, art. 60 §11-A Lei 8.213. Cruza com auditoria-laudo-pericial, analise-documental-incapacidade, base-incapacidade-b31-temporaria, base-incapacidade-b91-permanente, base-auxilio-acidente-b94-pos-reforma, analise-bpc-loas, aposentadoria-deficiencia, peticao-previdenciaria."
---

# Validação Formal de Relatório Médico. Checklist A e B do Escritório

## 1. Quando acionar esta skill

Acione SEMPRE que houver necessidade de validação formal de relatório médico para fins previdenciários ou assistenciais.

Hipóteses típicas. Análise de relatório médico para protocolo de B31, B91, B92, B94. Análise de relatório médico para BPC. Análise de relatório médico para aposentadoria PCD (LC 142/2013). Auditoria de laudo defeituoso. Refazimento de laudo após indeferimento.

A skill é fonte primária pró-segurado para distinguir os Checklists A (incapacidade laboral) e B (deficiência funcional para PCD/BPC), evitando confusão entre os dois universos probatórios.

## 2. Distinção entre os Checklists A e B

### 2.1. Checklist A. Incapacidade laboral

Aplicável a B31, B91, B92, B94 (com particularidades para auxílio-acidente).

Foco. Incapacidade para o trabalho habitual ou para qualquer atividade que garanta subsistência.

Eixo central. DII (data de início da incapacidade), DID (data de início da doença), prognóstico, sequelas, posologia.

### 2.2. Checklist B. Deficiência funcional

Aplicável a BPC (LOAS) e Aposentadoria PCD (LC 142/2013).

Foco. Impedimento de longo prazo (≥ 2 anos) que, em interação com barreiras, dificulta a participação plena e efetiva na sociedade.

Eixo central. DID (data de início da deficiência), funcionalidade conforme CIF, domínios afetados, IF-BrA ou IFBrM.

### 2.3. Confusão frequente

Erro comum. Médico assistente redige laudo de "incapacidade" para fins de BPC ou aposentadoria PCD, sem abordar a funcionalidade.

Erro comum reverso. Médico redige laudo de "deficiência" para fins de B91, sem caracterizar a incapacidade.

Solução. Solicitar laudo segmentado conforme finalidade, com checklist específico.

## 3. Marco normativo

### 3.1. Atos médicos e CFM

Lei 12.842/2013 (Lei do Ato Médico). Art. 1º. Define o ato médico.

Resolução CFM 1.851/2008. Atestado médico para fins de avaliação ocupacional.

Resolução CFM 2.314/2022. Disciplina a telemedicina.

Resolução CFM 2.057/2013. Atestado e laudo médicos. Validade.

Código de Ética Médica (Resolução CFM 2.217/2018). Arts. 80 e 81.

### 3.2. Normas previdenciárias

Lei 8.213/91, art. 60, § 11-A. Análise documental.

IN 128/2022, arts. 286, 287, 290. Documentos comprobatórios da incapacidade.

Portaria Conjunta MPS/INSS 13/2026. Análise documental B31.

Portaria Conjunta MPS/INSS 14/2026. Análise documental B91.

Portaria Conjunta MPS/INSS 15/2026. Análise documental B94.

### 3.3. Normas BPC e PCD

Lei 8.742/93 (LOAS).

Decreto 6.214/2007.

Portaria Conjunta SPS/INSS/SNAS 2/2014. Avaliação biopsicossocial.

Portaria 34/2025. TCQ.

Portaria 37/2026. IFBrM.

Lei 15.157/2025. Dispensa de reavaliação.

LC 142/2013. Aposentadoria PCD.

Portaria Interministerial AGU/MPS/MF/MP/PR 1/2014. IF-BrA.

## 4. Checklist A. Validação de laudo para incapacidade (B31, B91, B92, B94)

### 4.1. Cabeçalho

Item A.1. Nome completo do paciente.

Item A.2. CPF do paciente.

Item A.3. Data de nascimento.

Item A.4. Endereço residencial.

Item A.5. Identificação do médico (nome, CRM, especialidade).

Item A.6. Nome completo da clínica ou hospital.

Item A.7. Endereço da clínica ou hospital.

Item A.8. Data exata de emissão do laudo.

### 4.2. Diagnóstico

Item A.9. CID-10 ou CID-11 expresso.

Item A.10. Descrição do quadro clínico.

Item A.11. Sintomas atuais detalhados.

Item A.12. Histórico da doença (DID).

### 4.3. Curso da doença

Item A.13. Data de início da doença (DID) com fundamentação documental.

Item A.14. Data de início da incapacidade (DII) com fundamentação.

Item A.15. Evolução da doença (estável, progressiva, regressiva).

Item A.16. Tratamentos realizados.

Item A.17. Tratamentos atuais e posologia.

Item A.18. Prognóstico (favorável, reservado, desfavorável).

### 4.4. Funcionalidade

Item A.19. Limitações funcionais para o trabalho habitual.

Item A.20. Limitações funcionais para outras atividades.

Item A.21. Repercussão na vida diária.

Item A.22. Necessidade de auxílio de terceiros (B91 com adicional 25%).

### 4.5. Exames complementares

Item A.23. Lista de exames realizados com datas.

Item A.24. Resultados dos exames.

Item A.25. Anexos dos exames.

### 4.6. Conclusão

Item A.26. Caracterização expressa da incapacidade (temporária ou permanente).

Item A.27. Período estimado de incapacidade (B31).

Item A.28. Insuscetibilidade de reabilitação (B91).

Item A.29. Nexo causal: para B92, nexo com o trabalho (arts. 19 a 21-A da Lei 8.213/91); para B94, basta acidente de QUALQUER natureza (art. 86; Tema 416 STJ) — nexo laboral só quando se pleiteia a natureza acidentária/competência estadual.

Item A.30. Caracterização da sequela (B94).

### 4.7. Encerramento

Item A.31. Assinatura do médico.

Item A.32. Carimbo com CRM.

Item A.33. Carimbo da clínica ou hospital.

## 5. Checklist B. Validação de laudo para deficiência (BPC, aposentadoria PCD)

### 5.1. Cabeçalho

Item B.1. Nome completo do paciente.

Item B.2. CPF do paciente.

Item B.3. Data de nascimento.

Item B.4. Endereço residencial.

Item B.5. Identificação do médico (nome, CRM, especialidade).

Item B.6. Nome completo da clínica ou hospital.

Item B.7. Data exata de emissão do laudo.

### 5.2. Diagnóstico

Item B.8. CID-10 ou CID-11 expresso.

Item B.9. DSM-5-TR (para autismo, transtornos mentais, dupla codificação).

Item B.10. Descrição do quadro clínico.

Item B.11. Histórico (DID).

### 5.3. Funcionalidade conforme CIF

Item B.12. Domínio das funções e estruturas do corpo.

Item B.13. Domínio das atividades.

Item B.14. Domínio da participação.

Item B.15. Fatores ambientais (barreiras e facilitadores).

Item B.16. Fatores pessoais.

### 5.4. Aplicação dos sete domínios do IF-BrA (LC 142/2013)

Item B.17. Domínio Sensorial.

Item B.18. Domínio Comunicação.

Item B.19. Domínio Mobilidade.

Item B.20. Domínio Cuidados pessoais.

Item B.21. Domínio Vida doméstica.

Item B.22. Domínio Educação, trabalho e vida econômica.

Item B.23. Domínio Socialização e vida comunitária.

### 5.5. Aplicação do IFBrM (BPC)

Item B.24. Domínio físico.

Item B.25. Domínio mental.

Item B.26. Domínio intelectual.

Item B.27. Domínio sensorial.

Item B.28. Para menores de 16 anos. Particularidades do IFBrM aplicado à faixa etária (Portaria 37/2026).

### 5.6. Impedimento de longo prazo

Item B.29. Caracterização do impedimento como de longo prazo (≥ 2 anos).

Item B.30. Prognóstico (com ênfase na permanência ou progressão).

Item B.31. Tratamentos realizados e respostas.

Item B.32. Tratamentos atuais e respostas.

### 5.7. Barreiras

Item B.33. Barreiras físicas.

Item B.34. Barreiras atitudinais.

Item B.35. Barreiras de comunicação.

Item B.36. Barreiras tecnológicas.

### 5.8. Conclusão

Item B.37. Caracterização expressa da deficiência conforme art. 2º da Lei 13.146/2015.

Item B.38. Caracterização do grau (leve, moderado, grave) para aposentadoria PCD.

Item B.39. Aplicabilidade do impedimento de longo prazo (BPC).

### 5.9. Encerramento

Item B.40. Assinatura do médico.

Item B.41. Carimbo com CRM.

Item B.42. Carimbo da clínica ou hospital.

## 6. Vícios mais comuns nos laudos médicos

### 6.1. Vício de generalidade

Laudo apenas afirma "paciente apresenta CID X.X e está incapacitado".

Refutação. Insuficiência probatória. Solicitar laudo detalhado conforme checklist.

### 6.2. Vício de DID inexpressa

Laudo não fixa data de início da doença ou da deficiência.

Crítico para. B91 (com adicional 25%). Aposentadoria PCD (DID retroativa). BPC (caracterização do impedimento de longo prazo).

Cruzamento com `formacao-documentacao-did-pcd`.

### 6.3. Vício de prognóstico vago

Laudo afirma "prognóstico reservado" sem fundamentação.

Solicitar fundamentação técnica.

### 6.4. Vício de funcionalidade não abordada

Para BPC e aposentadoria PCD, laudo deve abordar funcionalidade conforme CIF.

Para B91, laudo deve abordar funcionalidade laboral (limitações para o trabalho habitual).

### 6.5. Vício de validade temporal

Laudo emitido há mais de 6 meses pode ser considerado desatualizado.

Recomendação. Atualização periódica para casos complexos.

### 6.6. Vício de identificação do médico

Falta CRM, especialidade, nome completo.

### 6.7. Vício de assinatura

Falta assinatura ou assinatura ilegível.

Em casos de telemedicina, validade da assinatura digital.

### 6.8. Vício de exames não anexados

Laudo menciona exames mas não os anexa.

Solicitar todos os exames mencionados.

## 7. Estratégia de refazimento de laudo

### 7.1. Notificação ao médico assistente

Solicitar reformulação do laudo conforme checklist.

Apresentar modelo do checklist aplicável ao caso.

### 7.2. Pareceres técnicos complementares

Em casos complexos, solicitar parecer técnico de especialista.

Cruzamento com `formacao-documentacao-did-pcd`.

### 7.3. Telemedicina e laudo extemporâneo

Resolução CFM 2.314/2022. Telemedicina admitida.

Resolução CFM 2.057/2013. Validade do laudo extemporâneo.

Análise documental do INSS aceita laudos com validade temporal compatível com o caso.

## 8. Aplicação por espécie de benefício

### 8.1. B31 (auxílio por incapacidade temporária)

Checklist A.

Foco. Período estimado de incapacidade.

Cruzamento com `analise-documental-incapacidade` e `base-incapacidade-b31-temporaria`.

### 8.2. B91 (aposentadoria por incapacidade permanente)

Checklist A com ênfase na insuscetibilidade de reabilitação.

Cruzamento com `base-incapacidade-b91-permanente`.

### 8.3. B92 (aposentadoria por incapacidade permanente acidentária)

Checklist A com ênfase no nexo causal acidentário.

Cruzamento com `base-incapacidade-acidentaria-b92` e `ntep-nexo-acidentario`.

### 8.4. B94 (auxílio-acidente)

Checklist A com ênfase na sequela e na consolidação.

Cruzamento com `auxilio-acidente-b94` e `base-auxilio-acidente-b94-pos-reforma`.

### 8.5. BPC (LOAS)

Checklist B.

Foco. Impedimento de longo prazo (≥ 2 anos).

Cruzamento com `analise-bpc-loas`, `base-bpc-impedimento-longo-prazo`.

### 8.6. Aposentadoria PCD (LC 142/2013)

Checklist B.

Foco. Sete domínios do IF-BrA + DID retroativa.

Cruzamento com `aposentadoria-deficiencia`, `base-pcd-if-bra-metodologia`, `base-pcd-did-retroativa`, `formacao-documentacao-did-pcd`.

## 9. Particularidades por patologia

### 9.1. Autismo (TEA)

Dupla codificação. CID-11 6A02 + DSM-5-TR.

Niveis de suporte (1, 2, 3).

Comorbidades frequentes (TDAH, deficiência intelectual, ansiedade, depressão).

Cruzamento com `aposentadoria-deficiencia` e `analise-bpc-loas`.

### 9.2. Fibromialgia

CID-10 M79.7.

Lei 15.176/2025 reconhece como deficiência.

Critérios ACR 2010.

Cruzamento com `base-pcd-fibromialgia-lei15176`.

### 9.3. Doença mental grave

Esquizofrenia, transtorno bipolar, transtorno depressivo recorrente grave.

DSM-5-TR + CID-11.

Histórico de internações, posologia, eventos psicóticos.

### 9.4. Câncer

CID-10 categoria C.

Estadiamento, tratamentos quimioterápicos e radioterápicos.

Isenção de carência (art. 26, II, Lei 8.213/91).

### 9.5. HIV

CID-10 B20-B24.

Carga viral, CD4.

Isenção de carência.

## 10. Documentos auxiliares ao laudo

Exames laboratoriais.

Exames de imagem.

Receituários.

Atestados de internação.

Relatórios de fisioterapia, psicoterapia.

Histórico ambulatorial.

Cartão de vacinação (BPC para crianças).

### Requisitos obrigatórios x facultativos no Atestmed (Onda 72)

O art. 2º da Portaria Conjunta MPS/INSS 13/2026, conferido no DOU de 24/03/2026, exige documento oficial com foto (caput) e documentação médica legível e sem rasuras contendo cinco incisos. Identificação do requerente, data de emissão, diagnóstico por extenso OU código CID (alternativos), assinatura do emitente e identificação do emitente com registro no Conselho de Classe legível.

Prazo estimado em dias (art. 2º, §2º) e data de início do repouso (art. 2º, §1º c/c art. 4º, §1º) NÃO são requisitos obrigatórios. São, respectivamente, elemento facultativo e lacuna suprida por norma. Recomenda-se obtê-los por estratégia, para reduzir o espaço do art. 4º, §3º, mas sua ausência não é causa normativa de indeferimento.

Detalhamento e uso pró-segurado da correção em `analise-documental-incapacidade`, seção Requisitos REAIS do art. 2º.

## 11. Cruzamento com outras skills

`auditoria-laudo-pericial` para auditoria do laudo pericial.

`analise-documental-incapacidade` para a análise documental B31/B91/B94.

`base-incapacidade-b31-temporaria` para B31.

`base-incapacidade-b91-permanente` para B91.

`base-incapacidade-acidentaria-b92` para B92.

`base-auxilio-acidente-b94-pos-reforma` para B94.

`auxilio-acidente-b94` para B94.

`analise-bpc-loas` para BPC.

`aposentadoria-deficiencia` para aposentadoria PCD.

`base-pcd-if-bra-metodologia` para o IF-BrA.

`base-pcd-did-retroativa` para a DID retroativa.

`formacao-documentacao-did-pcd` para formação de documentação.

`base-bpc-impedimento-longo-prazo` para o impedimento de longo prazo.

`peticao-previdenciaria` para a petição.

`orientacao-cliente-pericia` para orientação ao cliente.

## 12. Hub de portarias

`base-pericia-medica-federal-telepericia` para regime PMF e Teleperícia.

`base-portarias-dpmf-inss-hub` para Portarias DPMF.

`base-bpc-aposentadoria-pcd-procedimentos` para procedimentos BPC e PCD.

## 13. Cruzamento com Onda 28 (Modelos Geradores)

Para GERAR o modelo de relatório médico antes de enviar ao médico assistente (e depois usar esta skill para VALIDAR o que o médico devolver), acionar.

`base-modelo-relatorio-medico-incapacidade-b31-b91-b92` para B31, B91, B92.

`base-modelo-relatorio-medico-auxilio-acidente-b94` para B94.

`base-modelo-relatorio-medico-aposentadoria-pcd-lc142` para aposentadoria PCD LC 142/2013.

`base-modelo-relatorio-medico-bpc-loas-deficiente` para BPC/LOAS.

Fluxo. Acionar skill geradora para preparar modelo personalizado. Enviar ao médico assistente. Médico preenche. Acionar esta skill validadora (Checklist A para incapacidade/B94, Checklist B para PCD/BPC) para auditar antes do protocolo administrativo ou judicial.

## 14. Fontes

Consulte os arquivos `references/FUNDAMENTOS-E-CENARIOS.md` e `references/JURISPRUDENCIA-E-REFUTACAO.md`.

## Médicos Conferentes, acionamento obrigatório (Onda 141)

O Checklist A e o Checklist B conferem a FORMA do relatório médico. O conteúdo clínico é do médico da área, despachado em paralelo, e é ele que diz se o relatório demonstra o que a tese precisa.

| Se os documentos tratarem de | Despachar ao agente |
|---|---|
| Coluna, lombar, cervical, hérnia de disco, ciática, estenose, artrodese, CID M40 a M54 | `base-conhecimento-inss:medico-coluna` |
| Joelho, menisco, ligamento cruzado, artrose de joelho, prótese, quadril, tornozelo, fascite plantar | `base-conhecimento-inss:medico-membro-inferior` |
| Ombro, manguito rotador, supraespinhal, bursite, capsulite, CID M75 | `base-conhecimento-inss:medico-ombro` |
| Punho, mão, dedo, amputação, túnel do carpo, tenossinovite, epicondilite, LER e DORT | `base-conhecimento-inss:medico-membro-superior` |
| Depressão, ansiedade, bipolar, esquizofrenia, TEPT, burnout, CAPS, CID F20 a F48 | `base-conhecimento-inss:medico-psiquiatra` |
| Fibromialgia, artrite reumatoide, lúpus, espondilite, artrose difusa, osteoporose | `base-conhecimento-inss:medico-reumatologista` |
| Visão monocular, cegueira, glaucoma, retinopatia, surdez, audiometria, PAIR, implante coclear | `base-conhecimento-inss:medico-sensorial` |
| Câncer, infarto, insuficiência cardíaca, AVC, epilepsia, Parkinson, DPOC, insuficiência renal, HIV | `base-conhecimento-inss:medico-doencas-graves` |

Havendo mais de uma área nos documentos, despachar até TRÊS agentes, os das áreas que decidem o caso. Os pareceres voltam em uma página cada, em linguagem simples, e o que eles apontarem entra no relatório de validação, separado do que é vício formal.

O parecer médico é instrumento INTERNO. Ele orienta a peça, alimenta quesitos e indica o que pedir ao médico assistente, e NÃO vai aos autos como prova nem substitui laudo de assistente nem assistente técnico habilitado.
