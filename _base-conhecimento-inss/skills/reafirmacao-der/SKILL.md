---
name: reafirmacao-der
description: Skill de reafirmação da DER (Data de Entrada do Requerimento) para concessão ou revisão de benefício mais vantajoso. Use SEMPRE que mencionar reafirmação da DER, DER reafirmada, Tema 995 STJ, Tema 334 STF, melhor benefício, benefício mais vantajoso, art. 577 IN 128, art. 176-D Decreto 3.048, art. 690 IN 77/2015, quatro hipóteses de reafirmação administrativa, art. 33 Portaria 993/2022, art. 34 Portaria 993/2022, reafirmação na análise inicial até a DDB, reafirmação em revisão administrativa, arts. 21 e 22 Portaria 996/2022, reafirmação de ofício pelo INSS, fato superveniente art. 493 CPC, reafirmação administrativa, reafirmação judicial, DIB reafirmada, efeitos financeiros reafirmação, juros mora reafirmação, honorários reafirmação, desaposentação vs reafirmação, PUIL 5001623-70, Enunciado 1 CRPS, Questão de Ordem 20 TNU, concordância formal reafirmação, prequestionamento reafirmação, ou planejamento previdenciário envolvendo mudança de DER. Acionar AUTOMATICAMENTE com peticao-previdenciaria, precedentes-previdenciarios, revisao-peticao, planejamento-previdenciario quando o caso envolver reafirmação. NÃO use para desaposentação (Tema 503/STF).
---

# Reafirmação da DER — Skill Hub

## 1. Conceito

Reafirmação da DER é o deslocamento da Data de Entrada do Requerimento para momento posterior, quando os requisitos para concessão do benefício (ou de benefício mais vantajoso) foram implementados após a DER original.

NÃO é desaposentação. A distinção é fundamental e deve ser feita expressamente em toda petição. A desaposentação (vedada pelo STF, Tema 503, RE 661.256) exige renúncia a benefício já concedido. A reafirmação opera como reconhecimento de fato superveniente, com previsão normativa expressa.

## 2. Marco Normativo

### Constituição e lei
- Art. 201, §1º, CF — direito adquirido à aposentadoria.
- Arts. 493 e 933, CPC/2015 — fato superveniente constitutivo, modificativo ou extintivo, inclusive em grau recursal.

### Regulamento
- **Art. 176-D, Decreto 3.048/99** (Decreto 10.410/2020) — reafirmação na via administrativa, antes da decisão do INSS, com concordância formal do interessado (inclusive eletrônica).
- **Art. 176-E, Decreto 3.048/99** — dever do INSS de conceder benefício mais vantajoso ou diverso do requerido.

### Instrução Normativa 128/2022
- **Art. 577, I** — dever do INSS de oferecer opção ao benefício mais vantajoso, com demonstrativos financeiros.
- **Art. 577, II** — reafirmação quando requisitos implementados após DER e antes da decisão, mediante concordância formal.
- **Art. 222, §3º** — opção pelo cálculo mais vantajoso, observada reafirmação da DER.
- **Art. 589, §1º** — revisão e alteração para espécie mais vantajosa quando não oferecida na concessão.

### CRPS — Enunciado 1 (DOU 12/11/2019)
- Inciso III — reafirmação da DER até o cumprimento da decisão do CRPS.
- Inciso IV — opção mediante expressa concordância, aplicável a todas as situações que resultem em benefício mais vantajoso.

[NÃO CONFIRMADO, Onda 122] A referência anterior a este enunciado como "antigo Enunciado 5" foi removida do título por falta de confirmação em fonte oficial. O catálogo conferido registra o Enunciado 1 no DOU de 12/11/2019 e o Enunciado 5 como matéria diversa, contribuinte individual em atraso, com alteração no DOU de 06/12/2021. Antes de afirmar renumeração, conferir no PDF consolidado do CRPS.

Texto literal dos quatro incisos em `base-precedentes-catalogo-vinculantes/references/CATALOGO-ENUNCIADOS-CRPS.md`, que é a fonte a transcrever em peça.

## 2-A. As QUATRO HIPÓTESES DE REAFIRMAÇÃO NA VIA ADMINISTRATIVA (Onda 102)

Divisão didática das hipóteses previstas nas normas infralegais do INSS, incorporada de material do Prof. Alexandre Triches apresentado no evento Estrategista Previdenciário (julho de 2026). Conhecer o limite temporal e o requisito de cada hipótese é o que separa a reafirmação bem pedida da perdida por prazo.

Taxonomia conferida. Portaria DIRBEN/INSS 993/2022 é o Livro IV, processo administrativo previdenciário. Portaria DIRBEN/INSS 996/2022 é o Livro VII, recursos. Coerente com a taxonomia corrigida da `base-portarias-dpmf-inss-hub`.

### Hipótese 1. Análise inicial do requerimento (art. 33 da Portaria 993/2022)

[CONFERIDO em texto literal, 30/07/2026]

Caput. "Em se tratando de análise inicial de requerimento de benefício de aposentadoria, na hipótese de reconhecimento do direito a mais de uma aposentadoria na DER, deverá ser oferecida ao segurado a opção pelo benefício que seja mais vantajoso."

§ 1º. Estende o caput às situações em que o direito a mais de uma aposentadoria for implementado APÓS a DER até a data do despacho do benefício (DDB), "devendo ser oferecido ao segurado a possibilidade de reafirmação da DER para esta data, observado que ela deve ser anterior a DDB".

§ 2º. Quando na DER o segurado NÃO satisfazia os requisitos mas os implementou depois, até a DDB, "deverá o servidor informar ao interessado sobre a possibilidade de reafirmação da DER para esta data", exigindo-se "a expressa concordância por escrito ou por meio digital com validação de acesso por senha, como no Portal Meu INSS".

Uso pró-segurado. O § 2º cria DEVER DE INFORMAR do servidor. Requerimento indeferido sem que a possibilidade de reafirmação tenha sido informada, havendo implemento até a DDB, é indeferimento com vício de instrução, arguível em recurso ao CRPS e no eventual MS. O limite temporal desta hipótese é a DDB.

### Hipótese 2. Revisão administrativa (art. 34 da Portaria 993/2022)

[CONFERIDO em texto literal, 30/07/2026]

Caput. "Na hipótese de o segurado ter implementado todas as condições para uma ou mais de uma espécie de aposentadoria na DER ou até a data do despacho do benefício, na forma do disposto no art. 33, e em não lhe tendo sido oferecido o direito de opção pelo melhor benefício, deverá ser garantido esta opção no requerimento de revisão."

§ 1º. "Com exceção da hipótese prevista no caput, NÃO CABE reafirmação da DER nos pedidos de revisão ou em decorrência de procedimento de apuração de indício de irregularidade, considerando tratar-se de procedimento exclusivo da concessão."

§ 2º. "A alteração da DER prevista no caput está limitada à data do despacho do benefício na concessão."

Leitura de rigor. A hipótese 2 é EXCEÇÃO estreita. Só cabe reafirmação em revisão quando o direito à opção pelo melhor benefício existia e não foi oferecido. Fora disso, o § 1º veda expressamente, e essa vedação é o argumento que o INSS usará, de modo que a peça deve demonstrar o enquadramento no caput desde o início.

[NÃO LOCALIZADO na Portaria 993/2022] A condição de o interessado não ter recebido a prestação previdenciária nem sacado PIS ou FGTS, mencionada no material de origem, NÃO consta do art. 34 nem de qualquer passagem da Portaria 993/2022 na varredura de 30/07/2026. A condição pode estar em outro diploma (a hipótese mais provável é a disciplina da desistência ou da renúncia ao benefício já concedido). Antes de afirmá-la em peça ou parecer, localizar o dispositivo exato. Enquanto isso, tratar como requisito NÃO CONFIRMADO.

### Hipótese 3. Via recursal (Enunciado 1 do CRPS e art. 22 da Portaria 996/2022)

[PARCIALMENTE CONFERIDO] O Enunciado 1 do CRPS já consta desta skill (seção 3) e do catálogo. A reafirmação na via recursal até o cumprimento da decisão do CRPS ou do acórdão é o que o material afirma, com apoio no art. 22 da Portaria 996/2022, cujo texto literal NÃO foi conferido nesta rodada. Conferir o art. 22 na Portaria 996/2022 antes de citá-lo com número. A tese em si, reafirmação possível na via recursal, é sustentada pelo Enunciado 1 do CRPS e pelo Tema 995/STJ, ambos já verificados na base.

### Hipótese 4. De ofício pelo INSS (art. 21 da Portaria 996/2022)

[NÃO CONFIRMADO] O material afirma que a reafirmação de ofício é OBRIGATÓRIA quando há apresentação de novos elementos enfrentados no acórdão, sem limitação dos efeitos financeiros, no curso do processo. Se confirmado, é a hipótese mais favorável ao segurado das quatro, porque dispensa provocação e não limita efeitos. Justamente por isso, exige conferência do texto literal do art. 21 da Portaria 996/2022 antes de qualquer uso em peça. Pendência de manutenção prioritária.

### Quadro de limites temporais

Hipótese 1, limite na DDB. Hipótese 2, limite na data do despacho do benefício na concessão (§ 2º do art. 34), com a vedação do § 1º fora do caput. Hipótese 3, limite no cumprimento da decisão do CRPS ou do acórdão (a conferir no art. 22). Hipótese 4, no curso do processo, sem limitação de efeitos financeiros (a conferir no art. 21).

### Nota de manutenção

Conferência de 30/07/2026 pela `pesquisa-jurisprudencia-chrome` e por fonte secundária de legislação. Arts. 33 e 34 da Portaria 993/2022 conferidos em TEXTO LITERAL. Arts. 21 e 22 da Portaria 996/2022 e a ressalva de PIS/FGTS seguem PENDENTES, e a skill não os afirma como certos. Fechar a conferência no primeiro uso, pelo Comet, e promover os itens.

## 3. Precedentes Vinculantes

### Tema 995/STJ (REsp 1.727.063/SP, Rel. Mauro Campbell Marques)
**Tese (22/10/2019, trânsito 29/10/2020)** — É possível a reafirmação da DER para o momento em que implementados os requisitos para concessão do benefício, mesmo que isso se dê no interstício entre o ajuizamento da ação e a entrega da prestação jurisdicional nas instâncias ordinárias, nos termos dos arts. 493 e 933 do CPC/2015, observada a causa de pedir.

**EDcl no REsp 1.727.063/SP (DJe 21/05/2020)** — esclarecimentos vinculantes sobre juros e honorários no cenário pós-ajuizamento (ver seção 4, Cenário C).

**Alcance literal** — A tese trata apenas da reafirmação APÓS o ajuizamento. A reafirmação durante o PA ou entre o PA e o ajuizamento não foi objeto do repetitivo, sendo disciplinada pela TNU e pelos TRFs.

### Tema 334/STF (RE 630.501, Rel. p/ Acórdão Marco Aurélio, 21/02/2013)
**Tese** — Para o cálculo da RMI, cumpre observar o quadro mais favorável ao beneficiário, pouco importando o decesso remuneratório ocorrido em data posterior ao implemento das condições legais para a aposentadoria, respeitadas a decadência e a prescrição.

Fundamento constitucional do direito ao melhor benefício. A TNU utiliza o Tema 334 como pilar para a reafirmação da DER na esfera administrativa.

### TNU — Tese consolidada (PUIL 5001623-70.2022.4.02.5005/ES)
**Rel. João Carlos Cabrelon de Oliveira, 06/12/2024**
"Não há óbice à reafirmação da data de entrada do requerimento administrativo para o momento em que o segurado implementou os requisitos para um melhor benefício, enquanto aguarda a análise do seu pleito na via administrativa."

Confirmado por pelo menos 7 julgados da TNU em 2025 (ver references/precedentes-reafirmacao.md).

## 4. Os Quatro Cenários — Efeitos Financeiros, Juros e Honorários

### CENÁRIO A — Reafirmação durante o PA (antes da decisão do INSS)
- **DIB** — data de implementação dos requisitos.
- **Efeitos financeiros** — desde a nova DIB.
- **Juros de mora** — se judicializado, a partir da citação. Na via administrativa pura, não há juros.
- **Honorários** — regra geral se judicializado; N/A na via administrativa.
- **Status** — CONSOLIDADO (TNU + TRF4 + TRF3).

### CENÁRIO B — Implementação entre o fim do PA e o ajuizamento
- **DIB** — data de implementação dos requisitos (posição majoritária TNU/TRF4). ALERTA — a 1ª Turma do STJ (AgInt no REsp 2.018.250/RS) fixou efeitos a partir da citação nessa hipótese.
- **Efeitos financeiros** — controversos. TRF4 fixou a partir da propositura da demanda. TNU admite desde a implementação.
- **Juros de mora** — a partir da citação (TNU, Processo 0003337-85.2017.4.03.6303, Rel. Giovani Bigolin, 25/06/2025).
- **Honorários** — devidos sobre parcelas vencidas até sentença/acórdão.
- **Status** — EM CONSOLIDAÇÃO. Risco de divergência STJ.

### CENÁRIO C — Reafirmação no curso da ação judicial (pós-ajuizamento)
- **DIB** — data de implementação dos requisitos.
- **Efeitos financeiros** — desde a nova DIB.
- **Juros de mora** — somente após o 45º dia sem implantação do benefício (EDcl no REsp 1.727.063/SP, aplicação analógica do art. 41-A, §5º, Lei 8.213/91).
- **Honorários** — descabidos quando INSS reconhecer procedência à luz do fato novo. Cabíveis quando INSS resistir, sobre parcelas vencidas até sentença (Súmula 76/TRF4) ou acórdão que reformar improcedência (Súmula 111/STJ).
- **Status** — CONSOLIDADO (Tema 995/STJ, trânsito em julgado).

### CENÁRIO D — Reafirmação para melhor benefício (segurado já tem direito na DER)
- **Base normativa** — Tema 334/STF + art. 577, I, IN 128 + Enunciado 1, IV, CRPS.
- **DIB** — data de implementação dos requisitos para o benefício mais vantajoso.
- **Honorários** — quando há reforma de improcedência, base até o acórdão.
- **Status** — CONSOLIDADO para o cenário administrativo (PUIL 5001623-70). Em evolução nos demais cenários.

## 5. Distinção com Desaposentação — Fundamentação Obrigatória

Em TODA petição que envolver reafirmação da DER para melhor benefício, fundamentar expressamente a distinção.

**Desaposentação (vedada)** — Renúncia a benefício já concedido, com saque das parcelas, para obtenção de novo benefício. Vedada pelo STF (Tema 503, RE 661.256, 26/10/2016).

**Reafirmação da DER (permitida)** — Reconhecimento de fato superveniente durante o trâmite do PA ou judicial. Tem previsão normativa expressa (art. 577, IN 128; art. 176-D, Decreto 3.048/99). Não exige renúncia nem desistência do benefício. Decorre do direito constitucional ao melhor benefício (Tema 334/STF).

A TNU enfrentou expressamente essa distinção no PUIL 5001623-70, afastando a equiparação.

## 6. Aplicações por Tipo de Benefício

**Aposentadoria por tempo de contribuição (regras de transição EC 103/2019)** — caso mais comum. Segurado não atingiu pontuação/tempo na DER mas completou durante o trâmite. Reafirmação para escapar do fator previdenciário ou atingir regra mais vantajosa.

**Aposentadoria especial** — segurado completa 25 anos de exposição durante o trâmite. TRF3 admitiu via MS (ApelRemNec 5007016-49.2019.4.03.6105). TRF4 admitiu com base no IAC nº 5 para motoristas.

**Aposentadoria rural por idade** — completamento da idade mínima (55F/60M) durante o trâmite.

**Benefícios por incapacidade** — aplicação limitada; relevante quando segurado recupera qualidade de segurado por novas contribuições durante o trâmite.

**Revisão de benefício já concedido** — cenário D. Admitido pela TNU no PUIL 5001623-70 durante o PA.

## 7. Estratégia Processual e Prequestionamento

Para arquivo detalhado de precedentes, consultar `references/precedentes-reafirmacao.md`.

### Prequestionamento obrigatório desde a inicial
1. Tema 995/STJ e seus EDcl.
2. Tema 334/STF (direito ao melhor benefício como direito constitucional).
3. Arts. 493 e 933 do CPC/2015.
4. Art. 577, I e II, da IN 128/2022.
5. Art. 176-D do Decreto 3.048/99.
6. Distinção expressa com desaposentação (Tema 503/STF).
7. Se aplicável, Questão de Ordem nº 20 da TNU (anulação para adequação).

### Concordância formal do segurado
OBRIGATÓRIA em toda petição ou requerimento que envolva reafirmação. Na inicial, incluir manifestação expressa do segurado autorizando a reafirmação. No PJe, declaração no corpo da inicial ou documento apartado com ID.

### Causa de pedir
O Tema 995/STJ exige observância da causa de pedir. Se a inicial pede espécie X e a reafirmação levar a espécie diversa, há risco de julgamento extra petita. Solução obrigatória — pedir subsidiariamente a concessão do benefício mais vantajoso, com reafirmação da DER se necessário.

## 8. Alertas Automáticos

**ALERTA — Efeitos financeiros variáveis** — Em TODA petição com reafirmação, identificar expressamente qual cenário (A, B, C ou D) se aplica e fundamentar os efeitos financeiros conforme o cenário.

**ALERTA — Juros no cenário C** — No cenário pós-ajuizamento, juros só após o 45º dia sem implantação. Avaliar se o cenário B (pré-ajuizamento) é estrategicamente mais vantajoso.

**ALERTA — Honorários no cenário C** — Se INSS reconhecer procedência à luz do fato novo, honorários são indevidos. Prejudica o patrono. Nos cenários A e B, honorários seguem regra geral.

**ALERTA — Inadmissão de PEDILEF** — Alto índice de inadmissão na TNU sobre reafirmação. Prequestionamento adequado desde a inicial é imprescindível.

**ALERTA — Decadência** — Em revisão com reafirmação de benefício já concedido, verificar prazo decadencial do art. 103, Lei 8.213/91. Acionar automaticamente a skill decadencia-revisao-previdenciaria.

**ALERTA — Tema 1124** — Verificar compatibilidade com prévio requerimento administrativo. Se reafirmação envolve documentos não apresentados ao INSS, há risco de extinção sem mérito. Acionar automaticamente tema-1124-instrucao-administrativa.

## 9. Integração com o Ecossistema de Skills

Esta skill DEVE ser acionada automaticamente quando:
- `peticao-previdenciaria` redigir petição de aposentadoria e houver indicação de que os requisitos foram completados após a DER.
- `revisao-peticao` auditar petição que envolva reafirmação da DER.
- `precedentes-previdenciarios` for consultada sobre Tema 995, Tema 334 ou melhor benefício.
- `planejamento-previdenciario` for acionada e houver cenário de reafirmação.
- `triagem-caso-novo` identificar caso com requisitos completados após a DER.

Esta skill DEVE acionar automaticamente:
- `precedentes-previdenciarios` para fundamentação jurisprudencial.
- `tema-1124-instrucao-administrativa` quando envolver documentação não apresentada ao INSS.
- `decadencia-revisao-previdenciaria` quando envolver revisão de benefício já concedido.
- `tributacao-beneficios-previdenciarios` quando houver atrasados significativos (risco de RRA/IR).

## MCPs da casa

Antes de redigir, consulte os três servidores locais do plugin, nesta ordem. Os três localizam e não conferem, e nenhum autoriza a marca [CONFERIDO].

Legislação. Todo dispositivo citado nesta skill se transcreve do MCP `normas`, por `obter_artigo` no identificador da norma e no número do artigo (exemplo, `lei-8213-1991` e `57`), lendo o campo `texto` e a última ocorrência de cada parágrafo. Para tese de direito adquirido, `redacao_na_data`, que responde por ano. A citação em peça exige a `fonte_oficial` que a resposta devolve. Detalhe em `base-legislacao-fontes-primarias`.

Jurisprudência do TRF3 e das Turmas Recursais. Localize pelo MCP `trf3`, com `buscar_acordaos_trf3` (consulta, `polo_recorrente`, `resultado`, `orgao_julgador` e datas) e `obter_acordao_trf3` no id devolvido. `resultado` e `polo_recorrente` são inferidos. O achado nasce [NÃO CONFIRMADO] e só entra na peça depois de aberto no portal do TRF3, na forma de `pesquisa-jurisprudencia-chrome`. TNU e CRPS ficam no MCP `iurisprudencia`.

Acervo do escritório. Consulte pelo MCP `acervo` o que o escritório já sustentou neste tema. Comece por `buscar_tese_acervo` com os termos centrais desta skill e, achando trecho útil, leia o argumento inteiro com `obter_trecho_acervo`. Para saber em que peças um precedente já foi usado, chame `precedentes_do_acervo` e leia o campo `corte` da resposta, porque o mesmo número de Tema existe em mais de uma corte. Detalhe em `base-acervo-escritorio`.

**Vedação.** O acervo existe para o advogado LER o que já sustentou. Reaproveitamento automático de texto de um cliente em peça de outro é VEDADO. O trecho é ponto de partida para redação nova, conferida contra os autos e contra a legislação vigente na data. O trecho é anonimizado, e o arquivo de origem não é.
