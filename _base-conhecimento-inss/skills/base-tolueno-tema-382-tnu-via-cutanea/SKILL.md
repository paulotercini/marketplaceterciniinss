---
name: base-tolueno-tema-382-tnu-via-cutanea
description: "Tolueno via cutânea, Tema 382/TNU PUIL 5012678, distinção do benzeno e enquadramento qualitativo do agente químico. Use SEMPRE que mencionar tolueno, Tema 382 TNU, PUIL 5012678, tolueno via cutânea, tolueno absorção dérmica, tolueno Anexo 13 NR-15, tolueno 78 ppm, ACGIH 20 ppm tolueno, distinção tolueno benzeno, LINACH, hidrocarboneto aromático tolueno, NR-15 Anexo 11 12 13, exposição cutânea tolueno, sapateiro tolueno, gráfica tolueno, oficina mecânica tolueno, indústria tintas tolueno, frentista tolueno, EPI tolueno, Tema 555 STF, Tema 1090 STJ, Súmula 9 STF, NHO-08, neurotoxicidade tolueno, fetotoxicidade tolueno, art. 57 §3º Lei 8.213, art. 58 PPP tolueno, IRDR 15 TRF4, PEDILEF 0136882, PEDILEF 5001032, Decreto 53831 codigo 1.2.10. Cruza com auditoria-ppp, base-especial-agentes-quimicos, base-especial-epi, defesa-probatoria-especial, peticao-previdenciaria, precedentes-previdenciarios."
---

# Tolueno via Cutânea. Tema 382/TNU PUIL 5012678 e Enquadramento Qualitativo

## 1. Quando acionar esta skill

Acione SEMPRE que houver exposição a tolueno como agente nocivo em pleito de aposentadoria especial ou tempo especial.

Hipóteses típicas. Sapateiro, gráfica, oficina mecânica, indústria de tintas, indústria química, frentista, polidor, soldador, ajudante de produção química, operador de máquina com solvente.

A skill orienta o enquadramento do tolueno DIANTE da tese desfavorável do Tema 382/TNU (ver seção 2), privilegiando a via quantitativa e as coexposições. Corrigida pela auditoria de 06/07/2026.

## 2. O Tema 382/TNU (CORRIGIDO PELA AUDITORIA DE 06/07/2026 — TESE ADVERSA)

AVISO DE CORREÇÃO. A versão anterior desta skill atribuía ao Tema 382/TNU uma tese favorável sobre o benzeno, com número de processo divergente. A conferência na fonte oficial (CJF) desmentiu essa premissa. Dados verificados a seguir; ver CATALOGO-COMPLEMENTAR-VERIFICADO.

PEDILEF 5012678-57.2022.4.04.7108/RS. Julgado em 15/04/2026, trânsito em julgado em 26/05/2026.

Tese fixada (literal, fonte CJF). "A exposição ao tolueno por via cutânea, inclusive na sua forma líquida, não autoriza o reconhecimento da atividade especial por análise qualitativa (Anexo 13, NR-15)."

O Tema 382/TNU trata do TOLUENO e a tese é DESFAVORÁVEL ao segurado. Não citar como fundamento do enquadramento qualitativo por via cutânea; tratá-lo como tese adversa a contornar.

Caminhos pró-segurado que permanecem. Primeiro, a via QUANTITATIVA, dose de tolueno acima do limite de tolerância da NR-15 aferida em PPP/LTCAT (a tese veda só a análise qualitativa cutânea). Segundo, a coexposição documentada ao BENZENO ou a outro agente cancerígeno do LINACH grupo 1, com a presunção qualitativa própria desse agente. Terceiro, os demais agentes concomitantes do ambiente (ruído, outros químicos). Quarto, período anterior a 29/04/1995 por categoria profissional (Decretos 53.831/64 e 83.080/79), quando couber. Quinto, EPI, a dúvida sobre a eficácia favorece o segurado (Tema 1090/STJ) e o creme de proteção não elide absorção respiratória concomitante quando a dose for o fundamento.

## 3. Distinção técnica entre tolueno e benzeno

### 3.1. Benzeno

C6H6.

LINACH Grupo 1 (cancerígeno comprovado para humanos).

NR-15, Anexo 13-A. Limite de tolerância 1 ppm (anteriormente 1 ppm também).

Lei 13.467/2017 e Portaria MTb 1.109/2016. Regulamentação específica do benzeno.

Reconhecido cancerígeno. Não admite exposição segura. Enquadramento qualitativo.

### 3.2. Tolueno

C6H5CH3 (metilbenzeno).

LINACH NÃO é classificação primária. ACGIH classifica como A4 (inadequado para classificação como cancerígeno em humanos).

NR-15, Anexo 11. Limite de tolerância 78 ppm para 48 horas/semana.

Anexo 13 da NR-15. Tolueno listado entre as substâncias com absorção também pela via cutânea.

ACGIH (2017). Recomendação de redução do limite para 20 ppm em razão de neurotoxicidade.

NHO-08 da Fundacentro. Procedimento de avaliação da exposição a vapores orgânicos.

Tolueno tem efeitos. Neurotoxicidade. Hepatotoxicidade. Fetotoxicidade (categoria 2 GHS). Irritação dérmica e ocular.

## 4. Tese central pró-segurado (REVISTA após o Tema 382/TNU)

A tese da presença qualitativa cutânea CAIU com o Tema 382/TNU (trânsito em 26/05/2026, ver seção 2). A tese central passa a ser a via QUANTITATIVA, dose de tolueno acima do limite do Anexo 11 da NR-15 comprovada em PPP/LTCAT, somada às coexposições (benzeno/LINACH, ruído) e à categoria profissional até 28/04/1995 (auditoria 06/07/2026).

Isso porque o Anexo 13 da NR-15 é taxativo na enumeração das substâncias com risco grave por absorção cutânea, e a exposição a essas substâncias não admite gradação por concentração no ar (a via cutânea independe do ar).

## 5. Enquadramento normativo do tolueno

### 5.1. Decreto 53.831/64

Código 1.2.10. Hidrocarbonetos. Tolueno enquadrado como agente químico.

Código 1.2.11. Resíduos de hidrocarbonetos.

Aplicação até 28/04/1995 (categoria profissional + agente).

Aplicação até 05/03/1997 (atividade nociva).

### 5.2. Decreto 83.080/79

Código 1.2.10. Hidrocarbonetos.

Código 1.2.11. Resíduos.

Aplicação até 05/03/1997.

### 5.3. Decreto 2.172/97 (e Decreto 3.048/99, Anexo IV)

Código 1.0.19. Hidrocarbonetos aromáticos.

Tolueno enquadrado como hidrocarboneto aromático.

Aplicação a partir de 06/03/1997.

### 5.4. NR-15

Anexo 11. Agentes químicos com limite de tolerância. Tolueno 78 ppm para 48 horas/semana.

Anexo 13. Agentes químicos com risco grave por absorção cutânea. Tolueno expressamente listado.

Aplicação. A NR-15 é norma técnica complementar ao Decreto 3.048/99 e ao Anexo IV.

## 6. Argumentação contra o INSS

### 6.1. Argumento 1. Vincular o tolueno ao regime do Anexo 13 (cutânea), não ao Anexo 11 (concentração no ar)

Tese. A NR-15 prevê dois regimes distintos para o tolueno. Anexo 11 trata da exposição respiratória e exige aferição quantitativa pelo limite de tolerância. Anexo 13 trata da absorção cutânea e dispensa quantificação.

ATENÇÃO, após o Tema 382/TNU a via do Anexo 13 (qualitativa cutânea) está vedada para o tolueno. O argumento migra para o Anexo 11, dose quantitativa, e para as coexposições (auditoria 06/07/2026).

### 6.2. Argumento 2. ACGIH 20 ppm como reforço subsidiário

Tese. O limite de tolerância da NR-15 (78 ppm) está desatualizado. A ACGIH recomenda 20 ppm desde 2017. Em medidas acima de 20 ppm, há nocividade comprovada.

Aplicação subsidiária. Quando o argumento principal (Anexo 13/cutânea) não for aceito, sustentar que o limite real de nocividade é 20 ppm conforme recomendação ACGIH atualizada.

### 6.3. Argumento 3. Co-exposição com outros aromáticos (benzeno, xileno, etilbenzeno)

Tese. Em ambientes industriais, o tolueno raramente aparece isolado. A exposição é geralmente conjunta com benzeno (LINACH 1), xileno e etilbenzeno.

Estratégia. Argumentar a co-exposição como agravante. Mesmo se o tolueno isoladamente não for cancerígeno, a presença de benzeno simultânea atrai a aplicação do Tema 382/TNU.

### 6.4. Argumento 4. Distinguishing do Tema 382/TNU para invocar a sua ratio decidendi

Tese. O Tema 382/TNU fixou a desnecessidade da aferição quantitativa para agentes cancerígenos (LINACH 1). Por aplicação analógica e pela ratio decidendi do julgado, agentes do Anexo 13 da NR-15 (absorção cutânea) também não exigem quantificação.

Distinguishing. O Tema 382 é aplicado ao benzeno, mas a sua ratio (desnecessidade de quantificação para agentes com absorção sistêmica grave) é aplicável ao tolueno do Anexo 13.

### 6.5. Argumento 5. Súmula 9/STF e Tema 555/STF (EPI)

Tese. EPI ineficaz para neutralizar agentes cutâneos do Anexo 13.

A absorção cutânea ocorre pelo contato direto com a pele. Luvas sintéticas comuns (látex, nitrila não certificadas) NÃO são eficazes contra hidrocarbonetos aromáticos. Apenas luvas de nitrila industrial específica para químicos, com certificação CA específica para o agente, podem ser eficazes.

Cruzamento com `base-especial-epi`.

### 6.6. Argumento 6. PEDILEF 0136882-37.2014.4.05.8300/PE e PEDILEF 5001032-25.2014.4.04.7204/SC

Tese SUPERADA. A jurisprudência qualitativa cutânea do tolueno foi encerrada pelo Tema 382/TNU (tese desfavorável, seção 2). Não citar esses PEDILEFs a favor (auditoria 06/07/2026).

ATENÇÃO. Verificar inteiro teor antes de citar com número de processo. PEDILEF 0136882 e PEDILEF 5001032 NÃO foram localizados em fonte primária oficial até a data desta skill. Verificar no portal da TNU.

## 7. Cenários práticos

### 7.1. Cenário A. Sapateiro

Atividade típica. Aplicação de cola de sapateiro (contém tolueno em proporções variáveis).

Estratégia. PPP com campo 15.3 = "tolueno" + campo 15.4 = "exposição cutânea ao Anexo 13 da NR-15" + campo 15.7 = "EPI ineficaz".

Cruzamento com `auditoria-ppp` para validação do PPP.

### 7.2. Cenário B. Gráfica

Atividade típica. Operação de máquinas com tinta off-set (contém tolueno como diluente).

Estratégia. PPP com indicação de manuseio de solventes + Anexo 13 + co-exposição com xileno e etilbenzeno.

### 7.3. Cenário C. Oficina mecânica

Atividade típica. Manuseio de gasolina (contém tolueno e benzeno) + lavagem de peças com solventes orgânicos.

Estratégia. Co-exposição com benzeno (Tema 382/TNU diretamente aplicável) + tolueno do Anexo 13.

### 7.4. Cenário D. Indústria de tintas

Atividade típica. Operação de máquinas com tinta esmalte sintético (contém tolueno como solvente).

Estratégia. Anexo 13 + ACGIH 20 ppm + co-exposição com xileno.

### 7.5. Cenário E. Frentista

Atividade típica. Abastecimento de combustível (gasolina contém tolueno + benzeno).

Estratégia. Tema 382/TNU para o benzeno (cancerígeno) + tolueno do Anexo 13 cumulativamente.

Cruzamento com `precedentes-previdenciarios` para o Tema 382 e com `base-especial-agentes-quimicos`.

## 8. Estratégia de auditoria de PPP

### 8.1. Verificar o campo 15.3 (agente nocivo)

Tolueno deve estar expressamente identificado.

Em caso de "hidrocarbonetos aromáticos" sem especificação, exigir retificação para identificar o tolueno.

Cruzamento com `retificacao-ppp`.

### 8.2. Verificar o campo 15.4 (limite de tolerância)

Se houver aferição quantitativa, comparar com NR-15 (78 ppm) e ACGIH (20 ppm).

Se não houver aferição quantitativa, sustentar a aplicação do Anexo 13 (qualitativa).

### 8.3. Verificar o campo 15.5 (técnica utilizada)

NHO-08 da Fundacentro é a técnica adequada para vapores orgânicos.

Outras técnicas devem ser questionadas.

### 8.4. Verificar o campo 15.7 (EPI eficaz)

EPI deve ser específico para hidrocarbonetos aromáticos. Luvas comuns NÃO são eficazes.

Cruzamento com `base-especial-epi`.

### 8.5. Verificar o campo de observações

Eventual menção a "exposição cutânea", "contato com a pele", "absorção cutânea" reforça o enquadramento pelo Anexo 13.

## 9. Estratégia processual

### 9.1. CRPS

Recurso ordinário com fundamentação em.

Anexo 13 da NR-15 (taxatividade).

Tema 382/TNU (ratio decidendi por analogia).

ACGIH 20 ppm (subsidiário).

Co-exposição com benzeno (quando aplicável).

EPI ineficaz (Súmula 9/STF, Tema 555/STF).

### 9.2. JEF

Petição inicial com auditoria do PPP. Pedido de prova testemunhal sobre o uso real do tolueno e ineficácia do EPI.

Cruzamento com `tempo-especial-peticoes-por-rito`.

### 9.3. Ação ordinária

Para casos de longa exposição com necessidade de prova pericial complexa.

Pedido de perícia técnica em laudos da empresa, eventualmente com IEAN ou perícia em similar (Súmula 198/TFR).

## 10. Documentos essenciais

PPP (Perfil Profissiográfico Previdenciário) com todos os campos preenchidos.

LTCAT (Laudo Técnico das Condições Ambientais do Trabalho).

PGR (Programa de Gerenciamento de Riscos) da empresa.

Fichas de produtos químicos (FISPQ) usados na atividade.

CTPS com a função desempenhada.

Registros de fornecimento de EPI.

## 11. Cruzamento com outras skills

`auditoria-ppp` para auditoria do PPP.

`base-especial-agentes-quimicos` para o regime geral dos químicos.

`base-especial-epi` para a tese de EPI ineficaz.

`defesa-probatoria-especial` para Tema 1090/STJ e standard probatório.

`peticao-previdenciaria` para a petição.

`precedentes-previdenciarios` para Tema 382/TNU, Tema 555/STF, Tema 1090/STJ.

`tempo-especial-peticoes-por-rito` para a transposição em peça por rito.

`retificacao-ppp` para a retificação.

`base-aposentadoria-especial-transicao-ec103` para a transição da aposentadoria especial.

## 12. Hub de portarias

Acionar `base-portarias-dpmf-inss-hub` para as Portarias DPMF aplicáveis.

## 13. O que NÃO está nesta skill

Análise de outros agentes químicos isolados (benzeno, xileno, etilbenzeno) em `base-especial-agentes-quimicos`.

Cálculo de RMI em `base-calculo-rmi-ec103`.

Estratégia genérica de aposentadoria especial em `peticao-previdenciaria`.

## 14. Fontes

Consulte os arquivos `references/FUNDAMENTOS-E-CENARIOS.md` e `references/JURISPRUDENCIA-E-REFUTACAO.md`.
