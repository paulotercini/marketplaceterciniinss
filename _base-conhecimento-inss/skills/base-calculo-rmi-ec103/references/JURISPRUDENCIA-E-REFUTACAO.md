# Cálculo da RMI Pós EC 103/2019 — Jurisprudência Verificada e Refutação

## 1. Precedentes vinculantes

### Tema 1102 STF (Revisão da Vida Toda) e modulação

Tese original. Permite a inclusão de contribuições anteriores a julho de 1994 no cálculo da RMI para benefícios em que o método alternativo seja mais vantajoso, desde que preenchidos requisitos específicos.

Modulação em 21 de março de 2024. O STF restringiu a aplicação, com impactos para benefícios concedidos ou revisados, exigindo análise caso a caso dos parâmetros fixados.

Fonte oficial em https://portal.stf.jus.br

Aplicação pró-segurado. Fundamento para revisão de benefícios concedidos antes de 13 de novembro de 2019, com DIB sob a sistemática da Lei 9.876/99, dentro dos limites da modulação.

### Tema 1070 STJ (Atividades Concomitantes)

Tese. Todas as contribuições de atividades concomitantes devem ser somadas para fins de cálculo da RMI, afastando interpretação restritiva anterior.

Fonte oficial em https://www.stj.jus.br

Aplicação pró-segurado. Fundamenta revisão de RMI em que contribuições de atividades concomitantes foram desconsideradas. Aplicável também no cálculo pós-reforma quando houver contribuições concomitantes a partir de julho de 1994.

### Tema 999 STJ (Exclusão do teto da RMI)

Tese. A RMI do benefício deve considerar valores integrais das contribuições, independentemente da limitação ao teto no momento do pagamento mensal.

Fonte oficial em https://www.stj.jus.br

Aplicação pró-segurado. Em ações revisionais, o teto de pagamento não reduz a base de cálculo original.

### Tema 334 STF (RE 630.501)

Tese. Direito ao melhor benefício, com aplicação da legislação vigente no momento em que preenchidos os requisitos.

Fonte oficial em https://portal.stf.jus.br

Aplicação pró-segurado. Permite comparativo de cálculo entre regras distintas para escolha da mais vantajosa.

### Integração ao salário-de-benefício. Art. 31 da Lei 8.213/91

O fundamento é LEGAL e direto, sem tema repetitivo. O art. 31 da Lei 8.213/91 determina que o valor mensal do auxílio-acidente integra o salário de contribuição para fins de cálculo do salário de benefício de qualquer aposentadoria. Não citar o Tema 862 aqui, porque ele é termo inicial.

### Tema 862 STJ. TERMO INICIAL do auxílio-acidente

Tese literal, conferida na página oficial de precedentes qualificados do STJ em 18/09/2026. "O termo inicial do auxílio-acidente deve recair no dia seguinte ao da cessação do auxílio-doença que lhe deu origem, conforme determina o art. 86, § 2º, da Lei 8.213/91, observando-se a prescrição quinquenal da Súmula 85/STJ."

Primeira Seção, leading cases REsp 1.729.555/SP e REsp 1.786.736/SP, julgados em 09/06/2021, acórdão publicado em 01/07/2021. **Trânsito em julgado.** A matéria subiu ao STF como Tema 1225, e o Supremo decidiu em 13/08/2022 que NÃO há repercussão geral por ser questão infraconstitucional, com trânsito em 15/09/2022, de modo que a tese do STJ está firme e definitiva.

**ALERTA de uso.** O Tema 862 NÃO trata de integração do B94 ao salário-de-benefício. Essa integração tem fundamento legal direto no art. 31 da Lei 8.213/91 e NÃO possui tema repetitivo próprio. Citar o Tema 862 para sustentar integração entrega ao INSS um erro de fundamento.

**Regra prática desfavorável, registrada com honestidade.** A descrição oficial do Tema 1225/STF registra que o STJ, na ementa do julgado, considerou como termo inicial a DATA DA CITAÇÃO do INSS quando não houver auxílio-doença prévio nem requerimento administrativo do auxílio-acidente. Havendo B31 anterior, vale o dia seguinte à cessação. Não havendo, o requerimento administrativo prévio passa a ser decisivo para não perder retroativos.

Fonte oficial em https://www.stj.jus.br

Aplicação pró-segurado. Fundamento para inclusão do auxílio-acidente na média da aposentadoria, elevando a RMI.

### Súmula 557 STJ (revisão pelo teto)

Tese. Aposentado sujeito à revisão de benefício pelo teto pode ter RMI ajustada a valores superiores ao teto vigente à DIB quando o teto é posteriormente elevado por emenda constitucional.

Fonte oficial em https://www.stj.jus.br

Aplicação pró-segurado. Revisão do teto das EC 20/98 e 41/03 continua viva para benefícios concedidos sob tetos antigos.

## 2. Refutação aos argumentos típicos do INSS

### Argumento 1 — Aplicação automática do percentual sem análise do tempo excedente

Argumento adversário. O percentual aplicado é o vigente na data do requerimento, sem análise de tempo excedente.

Refutação. Art. 26, §2º, I, da EC 103. A RMI cresce em 2% a cada ano excedente a 20 (homem) ou 15 (mulher). O cálculo deve incorporar cada ano adicional comprovado, inclusive com tempo especial convertido e reconhecimento judicial.

### Argumento 2 — Desconsideração de salários de contribuição concomitantes

Argumento adversário. Apenas as contribuições principais são consideradas no cálculo.

Refutação. Tema 1070 STJ. Todas as contribuições concomitantes devem ser somadas. Acionar `impugnacao-cumprimento-concomitantes`.

### Argumento 3 — Média reduzida por contribuições mínimas

Argumento adversário. As contribuições mínimas integram o cálculo, reduzindo a média.

Refutação. Art. 195, §14, da CF, com redação da EC 103. Contribuições abaixo do mínimo devem ser complementadas. Em caso de complementação, o valor elevado integra a média. Acionar `contribuicoes-complementacao-ec103`.

### Argumento 4 — Impossibilidade de inclusão de contribuições anteriores a julho de 1994

Argumento adversário. Salários de contribuição anteriores a julho de 1994 não integram a média.

Refutação. Tema 1102 STF, dentro dos limites da modulação. Cabível em benefícios concedidos até 13 de novembro de 2019 sem trânsito em julgado, observados os critérios fixados.

### Argumento 5 — Desconsideração do caráter acidentário na B91

Argumento adversário. A B91 é sempre calculada em 60% + 2% por ano excedente, sem integralidade.

Refutação. Art. 26, §3º, III, da EC 103 combinado com §2º, II. B91 acidentária tem RMI de 100% da média. Acionar `ntep-nexo-acidentario` para conversão e `auditoria-laudo-pericial`.

### Argumento 6 — Pensão por morte com RMI fixa de 60%

Argumento adversário. Pensão por morte é sempre 60%, independentemente de dependentes.

Refutação. Art. 23 da EC 103. RMI = 50% + 10% por dependente, limitado a 100%. Incluir todos os dependentes elegíveis. Acionar `pensao-por-morte`.

### Argumento 7 — Desconsideração do auxílio-acidente no cálculo

Argumento adversário. O auxílio-acidente não integra a média de aposentadoria.

Refutação. Art. 31 da Lei 8.213/91. O auxílio-acidente integra o salário-de-contribuição e é considerado na média, observada a legislação vigente. Acionar `auxilio-acidente-b94`.

### Argumento 8 — Aplicação do teto atualizado retroativamente de forma adversa

Argumento adversário. O teto vigente na DIB limita a RMI, sem revisão posterior.

Refutação. Súmula 557 STJ. A revisão do teto pelas EC 20/98 e 41/03 permite recálculo com incidência dos novos tetos. Preservação do direito à integralidade.

## 3. Estratégia integrada

Primeiro, confirmar CNIS completo sem pendências. Acionar `cnis-acerto-indicadores`.

Segundo, realizar cálculo independente da RMI. Simular todas as regras aplicáveis.

Terceiro, verificar inclusão de atividades concomitantes. Somar todas as contribuições.

Quarto, em benefício por incapacidade, verificar caráter acidentário. Acionar `ntep-nexo-acidentario`.

Quinto, em pensão por morte, verificar todos os dependentes elegíveis.

Sexto, em revisão, invocar Tema 350 STF e Tema 1102 STF dentro dos limites da modulação.

Sétimo, em cumprimento de sentença, conferir cálculo do INSS linha por linha. Acionar `impugnacao-cumprimento-concomitantes`.

## 4. Cláusulas doutrinárias de apoio

Frederico Amado, em Direito Previdenciário, sustenta a análise crítica da RMI e defende a integralidade do cálculo em favor do segurado.

Hugo Goes, em Manual, detalha a sistemática do art. 26 da EC 103 e adverte para a necessidade de simulação caso a caso.

Fábio Zambitte Ibrahim, em Curso, critica a redução do percentual base e defende o aproveitamento pleno do tempo excedente.

Wladimir Novaes Martinez analisa o impacto da nova fórmula sobre perfis de alta remuneração e defende a escolha estratégica entre as regras disponíveis.

O IBDP sustenta institucionalmente o direito do segurado à transparência e à revisão quando identificada fragilidade no cálculo da RMI.

## 5. Diligência de atualização

Revalidar modulação do Tema 1102 STF. Verificar novos repetitivos do STJ sobre cálculo pós-reforma e revisões específicas. Acionar `precedentes-previdenciarios`.
