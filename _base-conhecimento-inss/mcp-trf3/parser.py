...
def _sumula(id_fonte, c, agora):
    """Súmula do TRF3, no acervo 'sumulas'. Não tem processo, partes nem resultado, e por isso entra
    com o número no lugar do CNJ, o enunciado na ementa e a origem (precedentes e projeto) no teor."""
    numero = re.sub(r"\D", "", c.get("Número", "").splitlines()[0])
    if not numero or not c.get("Ementa"):
        raise LayoutMudou(f"súmula {id_fonte}: rótulos lidos = {sorted(c)}")
    origem = "\n".join(f"{r}: {c[r]}" for r in ("Precedentes", "Observações") if c.get(r))
    return {"id": id_fonte, "acervo": "sumulas", "numero_cnj": numero,
            "classe_sigla": "SUM", "classe_nome": "SÚMULA", "orgao_julgador": c.get("Órgão julgador", ""),
            "relator": _sem_cargo(c.get("Relator(a)")) or "", "relator_titular": None, "relator_acordao": None,
            "data_julgamento": _iso(c.get("Data")), "data_publicacao": _iso(c.get("Data da publicação")),
            "polo_recorrente": None, "resultado": None,
            "ementa_texto": _limpo(RE_QUALIF.sub("", c["Ementa"])), "e_caso_exame": None, "e_questao": None, "e_razoes": None,
            "e_dispositivo": None, "inteiro_teor": origem, "coletado_em": agora}
...
def extrair(resposta, acervo, so_previdenciario=True):
    ...
    for id_fonte, corpo in zip(partes[1::2], partes[2::2]):
        ...
        if tipo == "súmula":                         # a Súmula do TRF3 vem no mesmo acervo, em formato próprio
            docs.append(_sumula(id_fonte, c, agora))
            continue
        if tipo != "acórdão":                        # sobra o que a fonte ainda não classificou
            continue
